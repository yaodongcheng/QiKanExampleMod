...
| [🔴 主菜单与自定义 UI 层](Knowledge/骑砍2主菜单与自定义UI层.md) | 🔴 **主菜单数据源（`Module.CurrentModule` 的 InitialStateOption 列表）+ 官方刷新链（`InitialState.RefreshContentState()`）+ MCM 冲突坑（它 Harmony postfix 按固定下标插按钮 → 换短列表 = 越界崩）+ 自建 ScreenBase 界面三件套（prefab/VM/层管理/开战役顺序）** | **要在主菜单加东西（按钮/多级菜单/选剧本界面）之前必读**；踩到 `ArgumentOutOfRangeException @ MCM.UI...` 时按此定位 |

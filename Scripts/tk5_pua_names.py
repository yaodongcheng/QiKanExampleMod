...
# 🔴 2026-09-12 补记：想用**原版日志**（正常字、0 私用区）反查这条路**试过，是死路**——
#    DX 人物池 1304 条 vs 原版 1100 条，两池**顺序不同**、DX 还多出增补条目，
#    同槽不同人（实测同槽同名 0 命中、相邻名字也对不上）；按「X兵衛/X右衛門 前缀交集」
#    也只剩 `新` 一个弱候选。唯一出路是 **F_FONTT.tr5 的「码 → 字面」表**（见
#    `太阁5_数据文件破解笔记_tr5msg与字体`，尚缺最后一步）。
#    ⇒ 消费方（`gen_generic_hero_ids.py`）现用**纯 ASCII 码点占位** id（`xe413bee`），
#      字体码表破解后改一处即可全表重跑。
UNRESOLVED = {
    0xE413: "DX日志 id=1033 「?兵卫」（候选：権兵衛/弥兵衛 等，词库无一命中；原版日志已排除）",
    0xE427: "DX日志 id=1075 「?犬」（同类：犬之助/狗丸/戌藏 同块，字头未知）",
}

...
def clean_name(name: str) -> str:
    """去掉 {=xxx} key，取 English fallback 显示名。"""
    if not name:
        return ""
    m = re.match(r"^\{=[^}]*\}(.*)$", name)
...
def main():
    rows = []  # dict: id,name,type,culture,complex,loc:{id:scene}
    for fname, tag in FILES:
        ...
        for sel in root.iter("Settlement"):
            ...
            rows.append({"id": sid, "name": name, "type": ltype, "culture": culture,
    ...
    for r in rows:
        for s in r["loc"].values():
            scene_counter[s.split(":")[0]] += 1

    # ---- CSV（宽表，值内无逗号，多值用 |）----
    loc_ids = sorted({lid for r in rows for lid in r["loc"]})
    with open(OUT_CSV, "w", newline="", encoding="utf-8-sig") as f:
        w = csv.writer(f)
        w.writerow(["settlement_id", "name", "type", "culture", "complex"] + loc_ids)
        for r in rows:
            w.writerow([r["id"], r["name"], r["type"], r["culture"], r["complex"]]
    ...
    lines.append("> 场景名规则：`scene_name` 三档繁荣度（1/2/3）；`l:x/2:y/3:z` 表示三档不同。地点属性（indoor/进入权限）只看模板 `location_complex_templates.xml`。\n")
    lines.append("## 0. 总览\n")
    ...
    lines.append("## 1. 城镇（town_complex）\n")
    ...
    for r in sorted(rows, key=lambda r: r["id"]):
        ...
        lines.append("| %s | %s | %s | %s | %s | %s | %s | %s | %s | %s | %s |" % (
            r["id"], r["name"], l.get("center", ""), l.get("arena", ""), l.get("tavern", ""),
    ...
    for r in sorted(rows, key=lambda r: r["id"]):
        if r["type"] != "castle":
            continue
        l = r["loc"]
        lines.append("| %s | %s | %s | %s | %s |" % (r["id"], r["name"], l.get("center", ""), l.get("lordshall", ""), l.get("prison", "")))
    ...
    for r in sorted(rows, key=lambda r: r["id"]):
        if r["type"] not in ("town", "castle", "village"):
            ...
            lines.append("| %s | %s | %s | %s | %s |" % (r["id"], r["name"], r["type"], r["culture"], scene or "(无 Locations)"))

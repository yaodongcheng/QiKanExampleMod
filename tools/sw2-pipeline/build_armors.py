...
#    ⚠️ 这里填的是【子网格号】不是零件表 idx（两者不同，check_partition.py 踩过同一个坑）。
...
#
# 🔴 2026-09-20 用户裁定（归属完备性：源模型每个面片必须落在 头/兜/甲/武器 之一）：
#    · 织田信长 sub0 = **背后披风**（idx2，142 面，普查判「撒开布（披风/斗篷）——不收」）。
#      普查"不收"的理由是 T-pose 平铺的布重定向后会像帐篷 —— 但**不收 ≠ 可以丢**。
#      裁定：**先临时挂进甲保底，不许丢**；布料形态（`--cloth-hang` 那条路）下一步单独做。
#      ⚠️ 同号兄弟 `submesh_0..._0000.001` 是他的**武器**（mat_w_nobunaga0）→ 由 is_junk 按材质排掉，
#         不会被一起抓进来（已核 build_armor.py:426）。
#    · 杂贺孙一 sub6 = **长外套**（idx8，591 面里的 282 顶点）。该件是【头发 + 长外套】复合件，
#      挑件表只把它接进了 `hair` 列 → 头管线取走头发、把外套那半就地删掉，而甲的名单里没有它
#      ⇒ 外套整块消失（头管线日志原文：`剔非头部：meshnode_0006 丢掉 282/551 个顶点`）。
#      裁定：**复合件必须双挂** —— 头侧走 hair 列（照旧），甲侧在这里补上；甲管线自带的
#      `--drop-head-idx` 只会取"不绑头骨"的那一半 = 外套，与头侧天然互补，不会重复。
#      先例：秀吉 idx0/idx7、谦信 idx9/10/15 都是「兜列 + 甲名单」双挂。
FORCE_ARMOR = {
    "L05_kenshin": [9],
    "L02_nobunaga": [0, 8],        # 披风（09-20 保底）· idx13 腕甲+腰件（09-20 用户裁定）
    "L09_magoichi": [6],           # 长外套（09-20：复合件补挂甲侧）
    # 🔴 2026-09-20 用户逐件裁定（49 件归属，见 Debug/offline/外观批量导入/_ruling/_my_ruling.json）
    "L10_shingen": [3, 9],         # H 兜件的肩甲片 + A 待看件
    "L11_masamune": [0, 8],        # A 待看件 + H 兜件的肩甲片
    "L13_hanzo": [8],              # A 待看件
    "L38_tadakatsu": [3, 6, 8, 9],  # H 兜件肩甲片 + A 待看件 ×2 + R 领口件
    "L40_ieyasu": [4, 7],          # H 兜件肩甲片 + R 领口件
    "L42_nagamasa": [2, 7],        # R 领口件 + A 待看件（跨脖子复合件，甲取非头骨那半）
    "L46_kanetsugu": [3],          # H 兜件的肩甲片
    "L49_musashi": [5, 7],         # A 待看件（背心）+ S 半丢件（idx7 衣服那半）
    # ---- 2026-09-20 第二轮（S 类：件在头侧列里、甲没加载 → 头管线剔掉的那半没人要）----
    #      用户裁定：这些被剔掉的半都在腰/腿一带（衣服、腰带、下摆）→ 归甲（跟身体动）
    "L06_oichi": [4],              # 阿市 idx6
    "L07_okuni": [4, 5],           # 阿国 idx8 / idx9（腰上那两条红带）
    "L100_kojiro": [6],            # 小次郎 idx9
    "L101_katsuie": [8],           # 胜家 idx10
    "L10_shingen": [3, 7, 9],      # 信玄 idx10（甲+头双挂）
    "L13_hanzo": [5, 8],           # 半藏 idx10
    "L14_rammaru": [6],            # 兰丸 idx8
    "L39_inahime": [6],            # 稻姬 idx8
    "L41_mitsunari": [6],          # 三成 idx9
    "L44_yoshihiro": [3],          # 义弘 idx8
    "L45_ginchiyo": [8],           # 訚千代 idx11
    "L47_nene": [6],               # 宁宁 idx10
}

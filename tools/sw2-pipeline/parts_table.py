...
def build_head_args(key):
    ...
    # 🔴 件位顺序 = **face, mouth, eye**（2026-09-15 实机修正：原来写的 face,eye,mouth 是错的）。
    #    引擎给脸部件分配贴图**按子网格的位置算**（`Knowledge/蒂法换头工程.md` §13.7 ①）。
    #    两个"已知能用"的样本都是【嘴在眼前】：
    #      · 原版女头 `head_female_a`：`.2`=嘴(242v)  `.6`=眼(98v)  `.7`=睫(72v)
    #      · 蒂法 `head_tifa_a`      ：`.1`=嘴(1594v) `.2`=睫+影 `.3`=眼+眉
    #    战无2 原来建的 `.1`=眼(38v) `.2`=嘴(137v) —— **眼在嘴前** → 部件被认错：
    #    实机症状「嘴巴开花 + 眼睛没有眼球」（28 人全中，与性别无关）。
    #    ⚠️ 女头的完整布局是 4 件（脸/嘴/眼/睫）；战无2 源模型没有独立睫毛件，
    #       仍只出 3 件（脸/嘴/眼），第 4 格空着。
    parts = "face,mouth,eye"

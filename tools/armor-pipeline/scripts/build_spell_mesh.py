...
DEPTH_BELLY = 0.10 # 弧腹处的刃厚（m）—— 🔴 2026-09-23 改：**薄片**（原 0.50 = 实体弧）
DEPTH_TIP = 0.03   # 其余弧段的刃厚（m）—— 薄，读得出"刃"
# 🔴🔴 2026-09-23 用户裁定：**月牙改成"薄片 + 贴图出细节"**（照第三方剑气 mod `jianqi_ok` 的思路）。
#    理由：① 斩击波本来就该是薄片（迎风面大、侧面读得出刃）
#          ② 面数 30720 → 768（细节交给贴图，几何只负责轮廓）
#          ③ 自管实体路线**不需要多档 LOD**（场景实体不吃导弹那套"超表尾即剔除"）
#    ⚠️ 核（`lwn_yinmo_core`）已是**独立网格**（2026-09-22 拆分），所以"刃厚要嵌住核"那条旧约束作废 ——
#       飞行网格里没有核了，厚度纯粹是视觉。
#    ⚠️ 要更宽的斩击波 = 调 `BAND_BELLY` / `BAND_TIP`（径向宽度），**不是**调刃厚。
#    ⚠️ 参考件实测（SwordBeam `jianqi_ok`）：202 顶点 / 264 面 / 3.0×10.8×0.55 的弧形薄片，
#       尺寸靠"按最长边归一化"到目标值；我们**按米建模**（跨度 3.81 m）、原点在弧腹 ⇒ 更可控。
...
SEG_ARC = 64      # 弧向分段（薄片：64 足够；原 96 是给实体弧的）
SEG_BAND = 3      # 径向分段（薄片：3 圈够表达收束；原 20 是给实体弧的圆角）
...
# 🔴🔴 全局缩放（2026-09-22 立；2026-09-23 定稿回 1.0）—— 所有**线性尺寸**统一乘这个系数，形状与朝向完全不变。
#     ⓘ 当初 5.0 是**实验值**：用来做"放大 N 倍，看远处还会不会消失"（判断"看不见"是**太小**还是**硬性距离剔除**）。
#       实验结论 = 与大小无关，是**导弹渲染路径**在约 110 米外硬剔除网格（法印计划 §16.2 七次实机证据链）⇒
#       改走**自管实体**（场景实体）之后该问题消失，所以缩放实验到此为止，**定稿 = 1.0**。
#     1.0 = 原尺寸（跨度 ≈3.8 m）｜5.0 = 五倍（跨度 ≈19 m，实验态）

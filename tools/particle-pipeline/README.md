...
>
> **2026-09-24 晚 更新（第 2 轮：照参考分镜返工）**
> · **新增 `preview/sheet_stills.py`**（一次渲多时刻 → 拼 3×3 分镜图）：**自检闭环就靠它** ——
>   一帧一读图费上下文，拼成一张 = 一次看 9 格。跑法见文件头。
> · **诊断**：把我们的分镜与参考分镜（`output/yinmo_ref/seq/B_11-13s.png`、`C_13-15s.png`）逐格比，
>   结论 = **我们的效果"太白、太干净、黑烟规模太小"** —— 参考的身份特征是「黑烟为主体 + 红发光点缀」，
>   我们三段全是白球。
> · **改动**（emitter 15 → **16**，校验 16 emitter / 问题 0 条）：白热核缩到"只是个核"·
>   绯红盘加大加亮（让"红"成主体）· 蓄力段**新增远场暗丝** `dark_far_filaments`（参考 11.0–11.75s
>   从球心向外辐射数米的细暗丝，原来完全没有）· 爆开段压亮壳、抬黑烟（速率 110→340、速度 →2.6）·
>   残迹段黑烟成规模（smoke_puffs 速率 70→220、尺寸 →0.42、曲线顶 →3.0）+ 月牙亮带加厚（0.32→0.52）。
> · **产物**：`out/sheet_p3_seq.png`（含替身网格，对标参考分镜）· `out/sheet_p2_particles.png`（纯粒子本体）。
> · 🔴 **预览不等于实机**（§0）：这轮调的是**形态/尺度/密度/节奏**；颜色亮度最终要看引擎材质 ⇒ 编成资产进 ModKit 看。
>

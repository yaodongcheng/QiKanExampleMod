...
本线交付的是 **FBX + TRF（给骑砍用的动画资产）**。如果你只是想**看 / 解析** UE 工程里的资产
（蓝图结构、数据表数值、**动画通知的触发时间**、粒子参数……），走 **`tools/ue-dissect/`**
（全仓唯一的 UE 资产导出/解析底座，导出 T3D 文本，秒级无损）。两条线互补，别互相替代。

> 具体到 FCS：它的 275 条动画里 **274 条可导出**，而**法术相关的 28 条 Sequence 已全部导出并重定向完成**
> （见 `Knowledge/FCS详情解析/Animation/README.md` 的状态列 —— 该目录 gitignored，用
> `python tools/ue-dissect/detail_tree.py` 重生成）。

...
namespace TpacTool.Lib
{
	public class Material : AssetItem
	{
		...
		public override void WriteMetadata(BinaryWriter stream)
		{
			...
			{
				stream.Write(kv.Key);
				stream.Write(kv.Value.Guid);
			}
			stream.Write(AlphaTest);
			stream.WriteStringList(ShaderMaterialFlags);
			ExtraMaterialSettings.Save(stream, SubVersion);
		}
		...
		public class ExtraMaterialSetting
		{
			...
			internal void Save(BinaryWriter stream, uint subVersion = 2)
			{
				stream.Write(AreamapScale);
				stream.Write(AreamapAmount);
				stream.Write(DetailnormalScale);
				stream.Write(NormalmapPower);
				stream.WriteVec4(MeshVectorArgument);
				stream.WriteVec4(MeshVectorArgument2);
				stream.WriteVec4(MeshFactorColorMultiplier);
				stream.WriteVec4(MeshFactor2ColorMultiplier);

	...
	public class AssetPackage
	{
		...
		protected virtual void Load(BinaryReader stream, bool loadDataIntoMemory)
		{
			...
			for (int i = 0; i < resourceNum; i++)
			{
                ...
                // raw 元数据捕获(assetclone 保真直写用): 先整段读原文, 再从其副本解析
                var metaBytes = stream.ReadBytes((int)metadataSize);
                if (metaBytes.Length != (int)metadataSize && (assetItem is Material || assetItem is Texture || assetItem is Metamesh))
                    Console.Error.WriteLine($"rawmeta short: {assetItem.Name} want={(int)metadataSize} got={metaBytes.Length} pos={stream.BaseStream.Position}/{stream.BaseStream.Length}");
                assetItem.RawMeta = metaBytes;
                using (var ms = new MemoryStream(metaBytes, false))
                using (var mbr = new BinaryReader(ms))
                {
                    try
                    {
                        assetItem.ReadMetadata(mbr, (int)metadataSize);
                    }
                    catch (Exception ex)
                    {
                        // 解析器与该 item 元数据版本不匹配(库缺陷, 如部分老纹理段): raw 已完整捕获,
                        // 存档走 RawMeta 直写, 半残对象仅提供 index 字段(Name/Guid/Type/Virtual), 可容忍。
                        Console.Error.WriteLine($"meta parse skip: {assetItem.Name} ({assetItem.GetType().Name}) want={(int)metadataSize} got={metaBytes.Length} remain={ms.Length - ms.Position} msg={ex.Message}");
                    }
                }
		...
		public virtual void Save(BinaryWriter stream, int tpacVersion = TPAC_LATEST_VERSION)
		{
			...
			{
				var asset = Items[i];
				// raw 直写优先(源字节保真, assetclone/roundtrip 不丢字段); 无 raw(新建对象)才走对象序列化
				var metadata = asset.RawMeta ?? asset.WriteMetadata();

...
def main():
    ...
    if t_mode:
        # 🔴 2026-09-20 修：这一步原来**只翻脸壳件**，脖子件（`neck`，独立 part）漏在外面 ——
        #    新抠出来的脖子那 121 面一个都没翻 → 领口上方留下朝内面（实机=看不见=透背景）。
        #    实测宁宁：闸门⑤ 从 4 个朝内面涨到 6 个，全是新脖子件带进来的。⇒ 脸壳 + 脖子一起翻。
        for _role in ("face", "neck"):
            _ob = joined.get(_role)
            if _ob is None:
                continue
            _f, _sk = flip_inward_neck_faces(_ob)
            print("  翻颈部朝里面[%s]：%d 个（不翻的话实机=脖子正面一条黑缝/透背景）；孪生面豁免 %d 个"
                  "（双面副本朝内是设计如此，翻了会把薄片毁成单面）" % (_role, _f, _sk))

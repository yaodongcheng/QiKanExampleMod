...
| 🔴 **八轮体验优化（2026-08-10）** | ① **发消息自动滚底**：ExecuteSend 后 `ScrollToBottom()`（ValueFloat=MaxValue）——翻历史后发消息新消息必可见；② **「有新消息」提示条**：新消息到达且玩家不在底部（val < max-8px 容差）→ 输入区分隔线上方悬浮提示（`HasNewMessageHint` + `ExecuteNewMessageClick` 点击滚底清提示）；③ **滚动条缩短**：MessageScrollbarHolder `MarginBottom=74` 与消息流同高，不再伸进输入框区域 |
| 🔴 **九轮体验优化（2026-08-10）** | **新消息处理二分（用户澄清）**：玩家在底部（未上拉）→ 新消息到达时**主动 `ScrollToBottom()` 把新消息弹出来**（贴底闭环兜底，不弹提示）；玩家在上部（翻历史）→ **才弹「有新消息」提示**。判断 = `IsMessageAtBottom()` |
| 🔴 **十轮语义修正（2026-08-10，用户实测确诊）** | 🔴 **`ScrollbarWidget.ValueFloat` 的贴底语义（Bottom 对齐）**：引擎每帧 `offset = MaxValue - val`——**val≈0 = 贴底（offset=max，InnerPanel 底部=Clip 底部）**；**val=max 反而是顶部（offset=0）**。旧实现「滚底=设 val=max」实际跳到顶部（用户实测「发送没拉到底」）、「贴底判定 val≥max-8」恒 false（贴底时 val=0 → 每次发消息都弹提示）。修复 = `ScrollToBottom()` 设 **val=0**、`IsMessageAtBottom()` 判 **\|val\|≤8**；手动滚轮接管加 clamp [0,max]（方向与引擎公式逐字一致：引擎 offset+=delta ⇔ val-=delta，实机验证过方向正确） |
| 🔴 **十一轮语义再修正（2026-08-10，反编译确诊 + 用户复测）** | 🔴 **十轮结论是错的——贴底 = val=max（= offset=0 = Bottom 对齐自然位）**。证据链：① 反编译 `ScrollablePanel.UpdateScrollablePanel`：Bottom 对齐时 `offset = MaxValue - val`；② 贴底判定的**铁证 = 内容不溢出时全可见**（offset=0、val 被引擎强制 0）——offset=0 = 显示最新消息；③ 滚轮方向（十轮实机验证正确）：上拉=往历史，手动接管 `val -= delta` ⇔ 引擎 `offset += delta` ⟹ offset=MaxValue-val ⟹ **val=max = offset=0 = 贴底**。十轮为何误判：只观察了「内容不溢出」态（该态 val 恒 0 且全可见，误以为 val≈0=贴底），溢出后 val=0 → offset=MaxValue → 面板下移露出最旧消息——**「滚动条出现后发送没拉到底」根因**。**第二个坑（内容增长漂移）**：贴底时内容增长 → MaxValue 变大、val 不变 → offset 漂移出新消息（数值判底 val≥max-8 在增长同帧必误判）。修复 = ①`ScrollToBottom()` 设 **val=MaxValue**；②`IsMessageAtBottom()` 判 **val≥MaxValue-8**；③**贴底状态机** `_pinnedToBottom`：ScrollToBottom/下拉到底/切会话 → 锁定；上拉 → 解锁；锁定态每帧重 pin（Tick 里 `if(_pinned) ScrollToBottom()`），新消息/发送永远弹出；④`RefreshMessages` 的 hadNew 分支改用**锁定态**判底（不用数值——增长同帧 MaxValue 未更新）；⑤保留 clamp [0,max]。**教训：Bottom 对齐 + 内容不溢出时 val 恒 0，是最容易让人误判贴底语义的陷阱** |

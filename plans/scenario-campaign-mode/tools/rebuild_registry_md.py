...
part1.append('> 🔴 **本部分 = 唯一大翻译表**（策划配置式）：第一列太阁原词 → 我们侧名 → 类型/语义/参数/实现用法/状态，一行一个词条。'
             ...
             '01 validator 的注册表检查（域/属性/函数/动作）读同一份 CSV——改表 = 改 CSV + 重跑生成，三处不再漂移。'
...
part1.append('\n## 1.4 函数（9 个）\n')
part1.append(render_table(sec['函数']))
part1.append('\n## 1.5 特殊形态速查\n')
...
- **容器机制**：容器篩選/選擇/設定/排除/清理/排序（~15000 次合计）→ 首版静态引用 + 映射表，`pick` 函数后续扩展
...
# 3) 删除 四函数表 + 五动作表（## 四、 到 ## 六、 前）
t, n = re.subn(r'## 四、函数表（完整，关系判断——与属性区分：属性是单值、函数是关系）.*?(?=## 六、触发时机注册表)', '', t, flags=re.DOTALL)
...
t = t.replace('# 第二部分：逐项骑砍2 对应（映射状态全表）',
              '# 第二部分：机制定义（总表之外的机制权威）\n'
              '> 🔴 2026-08-26 重构：原「一域表/二属性白名单/四函数表/五动作表/七映射总表」已并入**第一部分总表**（类型/语义/参数/实现/状态列在 CSV 大表）；'
              '本部分只保留**机制性定义**（总表容纳不了的：Ctx 三档生命周期、trigger/facility 注册表、覆盖结论）。'
              '函数参数见第一部分 1.4，动作参数见 1.3 命令行的参数/实现用法列。')

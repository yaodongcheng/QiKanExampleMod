...
# 🔴 2026-09-01 补丁：值字符集补 `·・`——主命::外交·同盟（武士） 等带间隔号的域值
#   原字符集无间隔号 → 正则吃不到 → 域值表外静默漏词（外交·* 8 个主命基名全漏，自检也看不见）
...
NPC_TOKENS = {
    ...
    '頭目': 'boss',
    # 🔴 2026-09-01 补丁：剧本实证 6 个模板NPC 对话位（門衛×193 / 酒場的老闆娘×27 / 宿屋的老闆娘×9 /
    #   保鏢×291 / 備大將×420 / 劍術師範代×25）——与 TaikouHero template_* 实体配套；
    #   泛用女忍者 按 2026-09-01 用户裁定「并入女忍者」（taikouhero kunoichi_01 Alias），不单立枚举
    '門衛': 'gate_guard', '酒場的老闆娘': 'tavern_landlady', '宿屋的老闆娘': 'inn_landlady',
    '保鏢': 'bodyguard', '備大將': 'battalion_captain', '劍術師範代': 'sword_deputy_instructor',
}
...
DOMAIN_VAL_MAP = {
    ...
    # 🔴 2026-09-01 补丁：按 T5EvDatTbl 类型24 全链补三节（權大納言/太政大臣/關白）——
    #   原表缺链会让 官职链 `>=` 比较断档；关白=148、征夷大将军=149（数据表权威序）
    ('官職', '征夷大將軍'): 'seii_taishogun', ('官職', '左大臣'): 'sadaijin',
    ('官職', '右大臣'): 'udaijin', ('官職', '內大臣'): 'naidaijin',
    ('官職', '太政大臣'): 'daijodaijin', ('官職', '關白'): 'kanpaku',
    ('官職', '大納言'): 'dainagon', ('官職', '權大納言'): 'gon_dainagon',
    ('官職', '中納言'): 'chunagon',
    ...
    # 🔴 2026-09-01 补丁：目標據點（语料 ::目標據點 ×4）——与 主人公據點/發生據點 同类的据点目标槽
    ('據點', '目標據點'): 'Ctx::target_settlement',
...
RES_SETS = {   # 🔴 资源型枚举 token 清单（数据包资源；不列清单 = 万能接收器，自检失效）
    ...
    '事件ＣＧ': {
        ...
        # 🔴 2026-09-01 补充：语料 (事件ＣＧ,墨股築城)×5 / (事件ＣＧ,大阪築城)×6
        '墨股築城', '大阪築城'
    },
    '背景': {
    ...
    '模板NPC': {
        ...
        '琉球商人', '米屋的老闆', '賊', '頭目',
        # 🔴 2026-09-01 补充（与 NPC_TOKENS 配套）：剧本实证 6 个
        '門衛', '酒場的老闆娘', '宿屋的老闆娘', '保鏢', '備大將', '劍術師範代',
    },
    '觸發': {

...
- **词条翻译权威 = `16a-DSL翻译总表.csv`**（🔴 编号 16a = plan 16 的数据文件，与 plan 本体 `16-DSL注册表全表.md` 区分——文件夹惯例：字母后缀 = 主 plan 的子文件，先例 = 原 07a；840 行：域 42 / 属性 245 / 域值 372 / 命令 155 / 语法 19 / 谓词 7——🔴 全部为 TK5 语料词（太阁原词列零 `—`、零 `::`/`.`、零英文 token，2026-08-27 用户裁定：mod 原生 18 动作 + 10 mod DSL 谓词 token 移出 16a，权威 = 本文件 §六/§三）；**「语法」类别** = 条件子句/组合器/分支/循环/事件头字段 19 词（調查/ＡＮＤ調查/場合分歧/循環/發生條件…），命令区 155 = 真·命令（动作/演出/世界结算/容器）；列 = 类别·太阁原词·所属域·我们侧名·值类型·语义·参数·备注·频率·例句，🔴 2026-08-27 用户裁定列序：类别第一列（排序分区）、所属域第三列（属性挂哪类对象）、频率倒数第二、例句最后一列（TK5 原句示范））——**查询词条翻译只看这一处**，域/属性/域值/谓词/动作 token 全表都在 CSV。CSV 由 `tools/build_registry_csv.py` 程序化生成（复跑 `TK5AllEvents_merged.txt` + 映射字典）；改词条 = 改 `gen_registry_tables.py`/`build_registry_csv.py` 字典 + 重跑。🔴 **v2 结构性修复（2026-08-27）**：属性 = 单键属性名 + 多段侧名（`Hero.clan / Settlement.clan`，域错配由生成期自检按语料实际域拦截——旧版 大名家.本城 2298 次被登记成 人物域 Hero.home 导致下游全量待注册）；**域值区** = 身份枚举/狀況值/命名槽等词条（实体引用如 人物::伊藤總十郎 **不入表**，走翻译器名字表 + 确定性兜底）；**表外词条 = 生成器缺陷**，生成期全语料覆盖自检不绿即 exit(1)（除 StringId 实体外，语料每个提取词都有行）
...
> 关系判断（带参函数）——与属性区分：属性是单值、谓词是关系。🔴 **分工（2026-08-27 用户裁定）**：本表 = **mod DSL 谓词 token 定义（13 个）**——mod token 无 TK5 源词，CSV 太阁原词列只收 TK5 词，故 token 注册地在本表；16a CSV 谓词区 = **TK5 调用词翻译行**（外交同盟→isAllied 等 7 行：太阁原词列 = TK5 调用词，侧名列 = 本表 token）；hasCard/canMove/canAttack 3 个只有 TK5 调用词无手写剧本先例，token 定义在此补登（2026-08-27）。谓词 = **带返回值的函数**——返回值类型在 CSV 值类型列（外交同盟/外交感情 = **数字**，例句 `外交感情(...)+(10)` 做算术、`外交同盟(...)!=(2)` 与数字比较；其余 = 布尔），所属域列 = 语料调用方域。语法语义（条件求值、四形态判断）见 01。**新谓词 = 本表加行 + CSV 调用词翻译行（若 TK5 有源词）+ 实现（01 条件求值）**。
...
| 类型 | 值类型列写法 | 说明 | 例 |
|---|---|---|---|
| 布尔 | `布尔` | true / false | `(Hero::X.alive)` |
| 数字 | `数字` | 整数/数值（比较 `>`/`<`/`>=`/`<=` 仅数字） | `(Time::year)` |
| 字符串 | `字符串` | 自由文本（显示名、文字列槽） | `代入文字列` |
| 空 | `空` | null（無效） | `人物::無效` |

**二、枚举（受限值集合；值 = 英文 token 字面量，🔴 禁止中文/自由字符串（铁律 20）；值集合权威 = 16a CSV 域值区 + §四 枚举注册表）**：
...
| 物品類型 | ❌ | 茶器 / 武器… | 物品数据包 |
| 狀態標誌族 | ❌ | 出現標誌(未出現/已出現)、出撃標誌(出撃中)、死刑標誌(死刑)、生病標誌(生病)、失蹤標誌(失蹤)、離家標誌(離家)、外出禁止標誌(真偽)、仇敵標誌(真偽)… | 引擎属性查询器 |
...
| 卡 | `对象:卡` | `Card::` | 卡/流派 | 技能卡数据包（流派放弃招式，称号层走 Card） |
...
**类型规则**：
1. 值类型列写法 = 上表「值类型列写法」（属性/域值/代入槽/谓词返回值 全表一致）
2. 赋值/比较两侧同型（见下「值类型一致性纪律」），validator 阻断违规
3. 枚举值 = 英文 token 字面量（§四），禁止中文/自由字符串（铁律 20）；带序枚举 `>=`/`<=` 顺序由 17 系统定义
4. 对象引用 = StringId（铁律 20），实体查名字表（人物::X → HERO_MAP），不在 16a CSV（2026-08-27 用户裁定）
...
| 谓词调用 `属性(参数)` | 参数按谓词注册表参数类型校验 | `全城壓制(國::甲斐, 大名家::X)`——region, clan ✓ |

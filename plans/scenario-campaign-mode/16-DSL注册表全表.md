...
| 域 | 属性 | 类型 | 语义 |
|---|---|---|---|
| Flag | value | 布尔 | `(Flag::okehazama_started) == true` |
| Variable | value | 数字/字符串 | `(Variable::某计数) >= 3` |
| Ctx | 按需 | 任意 | 🔴 上下文变量（见第三节）：`(Ctx::A.clan) == (Clan::clan_oda_1)` |
...
- **设计**：剧本上下文变量 `Ctx::A` / `Ctx::B` / `Ctx::C` / `Ctx::D` / `Ctx::E` + 语义槽 `Ctx::主人公` / `Ctx::發生據點` / `Ctx::主人公據點` / `Ctx::發生人物`
...
- **太阁映射**：代入人物Ａ → `ctx_set A`；主人公 → `Ctx::主人公`；發生據點 → `Ctx::發生據點`
...
| `調查:(據點::發生據點)!=(據點::那覇之町)` | `(Ctx::發生據點) != (Settlement::那霸町ID)` |
| `調查:(人物::主人公)==(人物::德川家康)` | `(Ctx::主人公) == (Hero::德川家康ID)` |
| `調查:(大名家::武田家.存在)` | `exists(Clan::武田家ID)` |
| `調查:(人物::X.親密度)>=(50)` | `hasRelation(Hero::X, Ctx::主人公, >=, 50)` |

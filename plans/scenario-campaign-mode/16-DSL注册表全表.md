...
| npc_talk | 人物對話時 | 🔴 03 对话钩子（原版对话流开场判定，见 [原版对话流引擎逆向分析](../../Knowledge/原版对话流引擎逆向分析.md)） | 77 事件——v1 漏登记里最大的一个 |
| battle_decided | 合戰決定時 | 🔴 02 锁军拦截（会战「打不打」拍板之后、进战场之前） | 7 事件；与 field_battle_start 的区别 = 这时还没进战斗场景 |
| clan_destroyed | 大名家滅亡時 | ✅ `CampaignEvents.OnClanDestroyed` | 2 事件 |
| monthly_forced | 每月處理的最後絕對 | 🔴 同 monthly 自建钩子，但**不参与互斥选路**（「絕對」= 全部满足条件的都执行） | 10 事件；validator 需按「不选路」处理 |
| travel_screen_select | 選擇移動畫面時 | 🔴 **无原生对应**（点开移动画面那一下）→ 降级 daily + 条件 | 17 事件；travel_screen 是画面显示完，这个是点开的瞬间 |
| facility_select | 選擇設施時 | 🔴 03 设施菜单点击钩子（点设施图标的瞬间） | 9 事件；house_enter 是进去之后，这个是点的瞬间 |
| army_move_start | 軍團移動開始時 | 🔴 **02 PartyBrain 出发检测**（与 army_move_end 同一套受控集合 tick） | 9 事件 |
| game_over | 遊戲結束時 | 🔴 mod 败亡结算自定义事件 | 2 事件；**≠ game_clear**（那个是通关） |
...
| smithy_town | 鍛冶屋 | 3 | 城里的公用铁匠铺（≠ 主人公鍛冶屋 = smithy） | 🔴 降级 |
| pirate_den | 海賊宅 | 3 | 同上 | 🔴 降级 |
| rice_shop | 米屋 | 2 | 同上 | 🔴 降级 |
| inn | 宿屋 | 2 | 同上 | 🔴 降级 |
| tea_master_house | 茶人宅 | 2 | 同上 | 🔴 降级 |
| ninja_house | 忍者宅 | 2 | 同上 | 🔴 降级 |
| overseas_trade | 海外交易所 | 2 | 同上 | 🔴 降级 |
| castle_drill | 城練兵場 | 1 | 城内练兵场 | 🔴 降级 |
| stable | 馬屋 | 1 | 同上 | 🔴 降级 |
| nanban_temple | 南蠻寺 | 1 | 南蛮寺 = 天主教堂 | 🔴 降级 |
| village_drill | 里練兵場 | 1 | 村里练兵场 | 🔴 降级 |
| village_training | 里修業場 | 1 | 村里修行场 | 🔴 降级 |
| fort_drill | 砦練兵場 | 1 | 砦 = 要塞 | 🔴 降级 |
| fort_training | 砦修業場 | 1 | 同上 | 🔴 降级 |
| shipyard | 造船所 | 1 | 同上 | 🔴 降级 |
| council_own | 主人公評定 | 1 | 主人公自己主持的评定（≠ 評定間 = 场所） | 🔴 降级 |
| pirate_manor | 海賊屋敷 | 1 | 同上 | 🔴 降级 |
| （不填 facility） | 無效 | 3 | 只写 trigger，不写 facility 字段 | ✅ |

**上表 = house_enter 第二参数的语料实测全量**（35 种 / 441 次，无省略）。设施词表本身还有 17 个 token 只出现在 `場面`/`背景`/`決鬥場地` 域、不作为 house_enter 参数：御所 / 忍屋敷 / 據點內畫面 / 發生設施 / 初期設定 / 合戰畫面 / 海道 / 賭博所 / 路口 / 陸道 / 黑暗 / 原野 / 忍者宅庭院 / 武家宅庭院 / 民家庭院 / 沙灘 / 船的甲板。

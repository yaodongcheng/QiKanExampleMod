...
| 值类型 | `ATTR_TYPES` + 规则推断（布尔/数字启发式）；🔴 仅 属性/域值 行有值（域/命令/函数行 = `—`，2026-08-27 用户裁定：与「类别」区分开） | ✅ 新属性类型 = 补 `ATTR_TYPES` | build_registry_csv.py `ATTR_TYPES` |
...
> 🔴 **带序枚举的序 = 显式等级链，禁止按枚举值排列推断（2026-08-27 用户裁定）**——`>=`/`<=` 比较依赖 17 系统定义的**独立等级映射**（每个值一个等级数：daimyo=10/city_lord=9/…），**不是** CSV 值集合的顺序、不是 token 字母序、不是注册表行序。**等级外值**（浪人/姑娘 等无等级身份）只有相等比较（`==`/`!=`），参与 `>=`/`<=` 视为违规（validator 阻断）。
...
2. 🔴 **域 = 容器，容器本身无类型（2026-08-27 用户裁定）**——域行值类型列 = `域`（标记）；**类型属于域值**（`事件標誌::38` = 枚举:旗标狀態 成立/不成立、`事件::X` = 枚举:事件完成狀態 已發生/未發生、`天氣::雲` = 枚举:天氣、`城::鳴海` = 对象:据点）

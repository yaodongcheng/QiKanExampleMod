...
## 七、交接：注册表查漏进度与待办（2026-08-27，新 session 从这里接手）

> **本节干什么用**：记录「扣除法查漏」这一轮做到哪、下一步做什么。做完一条划掉一条，全部做完删掉本节。
> **背景一句话**：用户裁定不再人工抽查漏洞——把语料复制一份，凡是注册表能解释的全部扣掉，扣不干净的就是漏洞（`tools/registry_residue_scan.py`）。🔴 **只对副本操作，原语料 `Knowledge/太阁事件包/TK5AllEvents_merged.txt` 只读。**
...
```bash
python plans/scenario-campaign-mode/tools/gen_registry_tables.py      # 生成期自检（表外 = exit 1）
python plans/scenario-campaign-mode/tools/build_registry_csv.py       # 重写 16a CSV
python plans/scenario-campaign-mode/tools/registry_residue_scan.py    # 扣除法残渣扫描（残渣 = exit 1）
```

- CSV：**1498 行**（域 42 + 属性 234 + 域值 573 + 枚举值 453 + 文本变量 12 + 命令 174 + 函数 10）
- 残渣扫描：**零残渣**；语料原文件未改动（`git status --short -- "Knowledge/太阁事件包/"` 为空）
- CSV 重复行组：**0**（此前 20 组双行已合并）

本轮修掉的漏洞（都属同一个病：**同一套词被写在两张表里，侧名各不相同**）：

| 病灶 | 修前 | 修后 |
|---|---|---|
| 身份 双表分叉 | 10 个词各有两个侧名（`ashigaru_captain` 一个名指两个品级） | 派生自域值表单一来源 |
| 場面 / 設施 / 背景 / 決鬥場地 | 四张表各写各的（城主間 = `lord_room` / hash / `lord_room`） | 共用 `PLACE_TOKENS`（53 词） |
| 軍團 / 軍團槽 | 主人公軍團 = `main_army` / `player` | 共用 `ARMY_SLOTS`（8 槽） |
| 觸發 22 词 / 模板NPC 13 词 | hash 侧名（`Trigger::tk5_uc82b44`） | 全部可读 token |
| 模板NPC 混入「其他」 | `其他` 被当成 NPC 模板 | 剔除（它是 主人公分歧 的 else 分支） |

新焊进生成期的自动断言（以后同类漏洞直接 exit 1，不用人查）：
① 词汇表内「一个侧名只能属于一个词」；② 派生集与域值表逐条一致；③ **跨表侧名分叉**——同一个 TK5 词在两张表里给出不同侧名即报错，合法的一词多义写进 `TWIN_EXEMPT` 白名单（现有两条：`終結`＝战斗结束/解散军团，`歸還`＝持续方针/一次性命令）。

### 7.2 待办 1（优先）：属性侧名塌缩 13 组 —— patch12 未做

**问题**：不同的太阁属性被映射到同一个我们侧字段，翻译器分不开、写回会串。上一轮的塌缩断言只查词汇表，**不查属性表**，所以漏了这一类。

| 我们侧字段 | 被挤在一起的太阁属性 | 判定 |
|---|---|---|
| `Hero.state` | 出現標誌 / 生病標誌 / 外出禁止標誌 / 死刑標誌 / 仇敵標誌 / 出撃標誌 / 失蹤標誌 | 7 个独立布尔，必须拆 |
| `Hero.merit` | 商人功勳 / 忍者功勳 / 武士功勳 / 海賊功勳 | 4 种职业功勋，必须拆 |
| `Hero.gold` | 所持金 / 貯金 | 随身钱 ≠ 存款 |
| `Hero.loyalty` | 忠誠度 / 義理 | 两种忠诚，义理另起 `Hero.honor` |
| `Settlement.kokudaka` | 基準石高 / 現石高 | 基准 ≠ 现值 |
| `Settlement.region` | 所在地方 / 所屬國 | 地方与国是两级行政区 |
| `Settlement.type` | 據點種類 / 據點類型 | **已验证取值空间不同**（0~3 vs 0~5），是两个属性 |
| `Item.price` | 價值 / 價格 | **已验证不是同义词**（2~7 品级 vs 5000~65535 钱） |
| `Settlement.materials` / `Army.materials` | 軍馬 / 鐵砲 | 两种军资，必须拆 |
| `Hero.appearance` | 容貌１ / 容貌２ / 容貌３ / 容貌４ | 编号被抹掉（见下） |
| `Army.general` | 武將２ / 武將３ / 武將４ / 武將５ | 编号被抹掉 |
| `Settlement.dojo_owner` | 道場主人 / 道場２主人 | 编号被抹掉 |

**根因（编号那三组）**：`gen_registry_tables.py` 的 `pair_side()` 里有条兜底规则，把属性名中的数字**直接删掉**再查表（武將２ → 查 武將），于是 2/3/4/5 号槽全变成同一个侧名。

**修法（已定，照做即可）**：
1. 编号规则改为**保留编号**：拿到基名侧名后追加 `_N`（武將２ → `Army.general_2`，道場２主人 → `Settlement.dojo_owner_2`，容貌１ → `Hero.appearance_1`）；全角数字先转半角。
2. 手写表（`PAIR_OVERRIDE`，它优先于旧表 `ATTR_MAP`）逐条补独立侧名：7 个标志改 `Hero.flag_*`；4 个功勋改 `Hero.merit_*`；貯金 → `Hero.savings`；義理 → `Hero.honor`；基準石高 → `Settlement.kokudaka_base`；所屬國 → `Settlement.province`；軍馬/鐵砲 → `*.materials_horse` / `*.materials_gun`；據點種類 → `Settlement.kind`；價值 → `Item.value_grade`。
3. 顺带把域表里 `國` 的侧名从 `Settlement.region` 改成 `Settlement.province`（域 `地方` 保留 region）。
4. **新增断言**：同一个域内，两个不同属性名共用一个侧名 = 报错（数据源用 `attr_pairs`，即语料实测的「域-属性」对）；留 `ATTR_SYNONYM_EXEMPT` 白名单给真同义词。

**不算漏洞、不要动的**：命令类别的「我们侧名」列是**处理器分组标签**（`se` / `rename` / `05 视觉`），多条命令共用一个处理器是设计如此；域类别的侧名是**命名空间前缀**（城/町/里/砦/據點 都是 `Settlement::`），共用合法。

### 7.3 待办 2：§二 触发/设施注册表补行

patch10 已在生成器里注册了新 token，**本文件 §二 的表还没同步**：

- **补 8 个触发**（语料里有、§二 v1 未声明）：人物對話時 `npc_talk`（77 次，最大的未声明契机）、合戰決定時 `battle_decided`、大名家滅亡時 `clan_destroyed`、每月處理的最後絕對 `monthly_forced`、選擇移動畫面時 `travel_screen_select`、選擇設施時 `facility_select`、軍團移動開始時 `army_move_start`、遊戲結束時 `game_over`。
- **补设施 token**：§二 现有一行「其余（醫師宅/職人宅/公家宅…）≤4 次 → 降级 menu_dialogue」的笼统兜底，需替换成显式 token 清单（生成器 `PLACE_TOKENS` 已注册 53 个场所，含背景专用与决斗场地专用）。
- **两处口径已按 §二 归一，需在表里标注**：城主間 `lord_room` → **`castle_hall`**、評定間 `council` → **`council_room`**（§二 已声明的 13 个设施 token 为权威，生成器已原样采用）。

### 7.4 待办 3：数字属性的"万能接收器"

`pair_side()` 里 `if attr.isdigit(): return f'{prefix}.attr_{attr}'` —— **任何**纯数字属性都能被接住（交易品::玻璃瓶.3 / 人物::X.205），意味着这块的自检等于没查。涉及 36 行 / 346 处。需要核对太阁属性表给出真实语义，再决定是收进专表还是保留兜底并**显式登记**。

### 7.5 剩余 hash 侧名 271 行 —— 是设计，不是漏洞

| 类别 | 行数 | 为什么留 hash |
|---|---|---|
| 事件ＣＧ / ＳＥ / ＢＧＭ | 73 / 72 / 28 | 纯媒体资产名，07 素材表落文件；patch11 已加兜底：名字若在别处注册过就复用其可读 slug |
| 主命 | 84 | 作者自定义具名实体，走确定性 ID + 中文名登记 |
| 儲存號 / 狀況 | 12 / 2 | 同上（存档槽变量名） |

🔴 **所有语义词表已零 hash**（修前是 396 行、15 个域/集）。

### 7.6 待确认（新 session 问用户）

本轮沉淀的模式是否登记为轮子进 `plans/rules/wheels.d/`：**扣除法自检**（复制语料 → 扣掉能解释的 → 残余即漏洞）＋ **词汇表单一来源**（一套词只在一处定义，其余表派生并各加前缀）＋ **孪生词表检测**（同词异名生成期报错）。

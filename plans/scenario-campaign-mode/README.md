...
6. **内容 = 剧本转化管线**（08：太阁5 事件包直译 + **任意题材原文直接提取成 01 DSL 剧本（无中间格式）**——三国演义走提取线；TK5AllEvents.txt 2594 事件 + 用户匹配资产库 taikou_final_v9_inference.xlsx）
...
5. **台词来源三态（2026-08-25 强化——用户裁定：默认原汁原味翻译，零发挥）**：🔴 **能翻译就翻译，别自己发挥**——台词是历史的（太阁5 原文），不是作者的（文笔）；【原文】= 太阁5 原句（可回查 `Knowledge/太阁事件包/TK5AllEvents.txt` 对应事件号）；【原文改编】= 仅允许称谓/报告者/场景的最小适配；【演绎补写】= **实在没办法才用**（如太阁5 的 AVG 表现部分骑砍2 确实表现不了），且必须标注；🔴 **禁止无标注的编造台词，禁止为了"剧情丰满"加戏**
...
| 08 转化管线 | 🔴 双源一线：太阁5 事件包直译（历史遗留翻译表）+ **原文直接提取成 01 DSL 剧本**（三国演义式，无中间格式）/时机·条件·指令映射/角色城池归一/**前置 mod 静态定义补全**（人城势力 XML，补充包 ModuleData）/预编译/产物校验 |
| 09 桶狭间剧本 | 完整剧本（演员/事件链/编排参数/演绎链/IF/验收）+ 🔴 多视角三通道（多角色名场面清单）；🔴 可执行级定义（事件骨架+演绎剧本+战斗预设+玩家表现逐行标注）见 [09b](09b-桶狭间剧本-可执行定义.md) |
...
- 09 事件链骨架化已产出（[09b 可执行定义](09b-桶狭间剧本-可执行定义.md)，待审核）；10 本能寺事件链骨架化待做（对照 09b 重写）

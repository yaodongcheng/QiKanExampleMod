...
7. 🔴 **织丰接入 = 方案 A 依赖织丰嵌入剧本层（2026-08-25 裁定，07）**：反编译实测推翻旧"1.5.1 硬失败"结论（程序集/API/运行时全兼容）；但真机实测织丰 DLL 报 dependency conflict（AssemblyLoader 引用循环找不到第三方 DLL，根因已定位）→ **ShokuhoTaikouExpansionPack 补充包**（独立 mod/仓库，同一工作区、打包分开）= 兼容层（AssemblyLoader 补丁 + 依赖预加载，排织丰前加载）+ 剧本层（入口/时间覆盖/行为注入）；方案 B 克隆注册层降为回退（织丰数据/兼容层均失败时）

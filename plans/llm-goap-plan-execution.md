...
> - 守卫模型泛化为 **ReactiveAgent**（任何 NPC 可当"对手/被叫者"，同一引擎）
> - 意外处理 = 引擎级 **GuardrailEngine 安全网** + 执行器状态机（Paused/Aborted/**Replan 低频重入**）
...
> 三个 case 不够——它们都只覆盖"引开型对抗"。真正的通用性判据：**把所有 case 摊开，每个 case 都能映射到四件套（意图分类/场景快照/计划执行/ReactiveAgent）+ 薄场景层上，且没有哪个 case 需要新引擎部件**。case 是验证手段，不是需求清单。

### 0.1 玩法 case 表（16 个）

| # | 玩家命令 | 意图解析 | 验证的框架部件 | 玩家体验 |
|---|---------|---------|---------------|---------|
| A | "我想偷那箱子，有人盯着怎么办？" | DISTRACT（守卫离开 watch_point）+ 玩家偷 | 标准对抗链路：引开型目标 + 窗口信号（GATE：`distance(guard, watch_point) > 10 sustained 5s` → 跳 s7"动手"）+ 折返预案（GATE：`following(guard, self) == false` → 跳 s8 秘密消息警报，双轨·特殊档，§5.1）——**两分支**：①跟走 → 窗口→动手；②不跟走（多疑）→ 再哄一次 / 放弃报告 | 守卫被引到门口 → "动手" → 玩家偷 / 多疑守卫不跟走："他不上当" |
| B | "请村长到我面前来" | BRING（村长到达玩家旁） | **合作型链**：随从 move_to 找到目标 → say_to 开口邀请 → 被叫方 ReactiveAgent 决策（duty/social 决定跟/拒；拒绝 = Y2）→ 跟来（follow 随从）→ 到达判定 GoalTemplate → 逗留窗口（下限 ~10s，人格修正；对话取消回岗） | 村长跟来 → 随从"村长来了" → 村长"找我什么事？"（逗留开口） → 对话/不理他自行回岗 |
| C | "我引开守卫，你去偷那箱子" | STEAL（随从偷）+ 玩家引开 | **分工**（通用机制，§2.2）：偷箱子 → 随从执行（actor=随从）；引开守卫 → 玩家执行（actor=player，玩家侧自理）+ NPC 偷窃原子行为 + 物品移交（铁律 4）+ 窗口谓词检测（`!seeing(any, self)` sustained——唯一安全条件，**GATE**：成立才动手，any = 任意会告发的目击者，判定对象是偷窃的随从本人而非箱子，§5.2；守卫脱身回看 = 条件自动翻转 → 触发预案） | 玩家纠缠守卫，随从偷箱子 → 物品移交 |
| D | "去把那个守卫的钱袋摸来" | STEAL（target=人） | 同一意图的 target 类型切换（物→人）+ **绕背定位**（`steal_attempt` 人变体内部几何搜索：目标盲区 + 无目击者视野；站位不可行 → 诚实报告，§4）+ 窗口 `!seeing(any, self)`（**GATE**：成立才动手，同 case C）+ 扒窃结算（**两分支**：到手 → 移交；摸空/目标察觉 → 空手收尾 + 报告） | 随从绕背扒窃，钱袋到手 / 摸空："没摸到，他太精了" |
| E | "缠住掌柜，我去翻那保管箱" | ENGAGE（掌柜滞留 + 看不到玩家） | **滞留 + 调视野双条件**：`!seeing(掌柜, player) sustained && !moving(掌柜)`（**GOAL + MAINTAIN**：达成后保持到玩家叫停，掉线 = 预案）——**目击判定是"小偷被看到"（玩家），不是"箱子被看到"**（§5.2）；站位决定视野（随从站保管箱对侧，掌柜背对箱子），验证 GoalTemplate 不只有"离开"一种形态 | 掌柜聊得走不开且看不到玩家，玩家翻保管箱 |
| F | "闹出点动静，把人都引过来" | COMMOTION（群体视线聚拢） | **单对多**：`seeing(all, companion)`（**GOAL + MAINTAIN**：达成后保持到玩家叫停，掉线 = 预案）——**故意被看到**（脱身/掩护场景：被看到是目的）+ 声东击西组合；复用 WitnessCrime 围观聚集（§10） | 随从砸翻酒桶，全店看过去，玩家下手 |
| G | "把那个守卫引到巷子里打晕" | KNOCKOUT（目标状态=击晕） | **意图单层、计划组合**：引开只是手段，目标始终一个（击晕）——计划链 = 诱骗跟走（say_to，同 case A 引开）→ 引到**隐蔽点**（`hidden_spot` 动态查询，§5.0：周围无人 + 无视野——mission 没有"巷子"标记，条件由引擎空间搜索满足）→ 窗口 `!seeing(any, self)`（GATE：持续验证，人会动）→ 绕背 `knockout` 原子行为（复用背后击晕轮子）→ GOAL：目标击晕；折返预案同 case A | 守卫被击晕在僻静角落，玩家大摇大摆 |
| H | "帮我望风，来人了叫我" | LOOKOUT（异常 → signal） | **事件驱动目标**（无 success 条件）+ 感知报告链——**TRIGGER 条件**：`in_zone(any, watch_zone)` 上升沿（任意非队友首次进入望风区）→ signal_player"有人来了！"，**报告后继续望风**（计划不结束，可重复触发）；玩家 R3 叫停 = 收尾（R6 豁免） | 随从门口望风，有人来就"有人来了！"（即时消息·特殊档——望风脱不开身） |
| I | "告诉他，我在老地方等他" | DELIVER（目标听到消息） | 无对抗纯行动 + 最小计划形态（move→say→回来报告）——GOAL：目标听到消息（say_to 完成）；**两分支**：①听到 → 回来报告"说好了"；②不理/赶走随从 → 回来如实报告（转述原话，台词一致） | 随从传话回来："说好了" / "他让我滚开"（返回当面说·默认档） |
| J | "带我去河边" | GUIDE（玩家到达目的地） | **目标参与者=玩家**（GoalTemplate 可引用玩家状态）+ **`lead` 原子行为**（§4：节奏同步在引擎内部——前进 + 定期回望，玩家跟丢即停下等，**不自顾自走**；玩家长时间不动 → 中止 + 报告） | 随从带路（走一段等一段），玩家跟着走到河边 |
| K | "把那个醉鬼从我身边赶走" | DRIVE_AWAY（醉鬼离开玩家 10m） | **威胁型中间态**（非合作非对抗）：say_to 恐吓 → 目标反应 = **剧本骨架 + 人格演算**（§6.4）——**两分支**：①**走开**（gullibility 高/temper 低）：`distance(target, player) > 10`（GOAL）→ 成功收尾；②**反抗**（temper 高）：骂回/挥拳 → R5 目标敌对 → Aborted/演变成战斗（铁律 12：恐吓失败有代价） | 醉鬼骂骂咧咧走开 / 或"你算老几！"挥拳相向 |
| L | "你在这等我，去那边看看有什么" | SCOUT（随从返回并报告所见） | **信息也是目标产物**：侦察往返 + 情报回报通道——GOAL：返回并如实报告所见（含**空情报分支**：没发现 → 报告"那边没人/没什么"） | 随从回来："巷子里有伙人" / "那边没什么" |
| M | "干掉他" | ATTACK（目标被击倒） | 战斗原子行为接入（`order_attack` 已有）+ 战斗风格参数（不留恋战斗）——GOAL：目标被击倒（`dead(target)`）；**两分支**：①击倒 → 收手；②打不过/目标逃跑 → 脱战回来报告（R1 受伤 → 护主/撤退） | 随从拔刀，干净利落 / 打不过："他太硬，我先撤了" |
| N | "把全村人都杀了" | ANNIHILATE（zone 内敌对清空） | **计划语法扩展**：循环段（loop/until：`all_in(zone)` 敌对 = 0）+ 动态目标引用（query：下一个敌人）+ **群体恐慌传播链**——**失败分支**：目标逃出 zone / 随从受伤（R1）→ 循环中止 + 报告战果 | 随从逐屋清剿，村民四散奔逃 / 逃了一批："跑了几个" |
| O | "他要是敢还手，你就上" | GUARD + 条件参战 | contingency 的战斗用法（combat(target) → attack）+ 既有护主机制——**MAINTAIN**：目标不动手 = 持续压阵（保持戒备跟随）；触发参战 → 战斗收手后恢复压阵 | 随从压阵，对方动手才拔刀 |
| P | "打晕门口那两个守卫" | 批量 KNOCKOUT | 循环段 + KNOCKOUT 组合（非致死批量）——GOAL：全部击晕（`all_in(zone)` 击晕）；**失败分支**：惊动增援/目标醒转 → 中止 + 报告 | 两个守卫被放倒，随从收手 / 惊动了："来了增援，先撤" |
...
| Y2 | 村长拒绝来 | 拒绝型反应（duty 高走不开 / social 低不给面子，§6.4 人格演算） | 拒绝台词 → 随从返回玩家旁 → 当面冒泡转述"村长说忙，不来"（报告双轨·默认档，§5.4）→ 计划收尾 |
...
**判据结论**：16+4 个 case 全部落在"意图词表一行 + GoalTemplate 一个 + prompt 描述一段"的薄场景层上——**没有任何 case 需要新引擎部件**。新增 case 的成本恒定（≈一个枚举行 + 一个谓词组合 + 一段 prompt），不随 case 数量增长——这就是框架通用性的定义。（战斗批量 case N/P 需要**计划语法扩展**：循环段 + 动态目标引用，见 §0.5 缺口⑤——这是语法能力成长，不是引擎重构。）

**真正的能力边界**（诚实列举，不是设计缺陷）：词表外的命令 → CUSTOM 诚实拒绝；需要引擎不存在的实体交互（INTERACT）→ 待验证后进词表；超出 Mission 层的能力（大地图指令）→ 不属于本系统。
...
| W6 | "假扮贩马商人，去套镖师的话" | TALK_TO（伪装 = 台词内容 + gullibility 判定） | **验证：欺骗是反应层问题，无需新引擎** |
...
### 0.5 压力测试结论：真缺口只有 4 个，且全是"库要长"不是"引擎要改"
...
**结论**：24 个 case 无一动引擎架构（四件套不变），缺口全是"词表与原子行为库的增长点"——玩法多样性的成本 = 词表行数，与框架复杂度解耦。两个额外收获：欺骗 = 台词 + 反应层判定（gullibility，无新引擎）；引导机制可复用给任何人（W7）。
...
| "切磋"要引擎驱动玩家 | 玩家被寻址时玩家侧自理 + 响应型计划（备战→等触发→结算） | §0.6 规则 3 |
...
CommandIntent { type, target, who_does, params }
  │  查 GoalTemplate 表（C# 薄层）→ 引擎可验证的目标状态定义
  ▼ ② 计划生成（LLM，同一次调用）
  ...
  GuardrailEngine 安全网（引擎级硬规则，所有计划共享）
    意外 → Paused（可恢复）→ Aborted（报告）→ Replan（低频 LLM 重入）
```

**GOAP 关系**：GOAP = 目标状态 + Action 集 + 规划器 + 执行器。本项目 = **LLM 替代规划器**（输出意图分类 + 全 cast 计划），**引擎做执行器**（确定性解释 + 安全网）。LLM 只写"程序"，不跑"程序"。

**泛化原则**：四件套通用引擎（意图分类、场景快照、计划语法+执行器、ReactiveAgent）+ 一层薄场景定义（GoalTemplate 表 + 角色表 + 意图词表描述）。未来命令 = 意图词表加一行 + GoalTemplate 加一个 + prompt 描述，引擎零改动。
...
| 意图 | 玩家话术 | 目标状态（GoalTemplate，引擎可验证） | 能力 | 阶段 |
|------|---------|--------------------------------------|------|------|
...
| GUIDE | 带我去河边 | `distance(player, destination) < 3`（GOAL：玩家到达目标点；Impossible = 目的地不可达预检） | `lead` 原子行为（§4：节奏同步在引擎内部，不自顾自走）+ signal | v2 |
...
| INTERACT | 把门打开/把灯吹灭 | 实体状态改变 | 实体交互 API（引擎能力待验证） | v3 |
| DISCREET | 低调点/别惹事 | 行为参数（不跑不叫不打架） | 行为参数化 | v2 |
| CUSTOM | 词表外的请求 | 引擎无法验证 → **诚实拒绝**："这我做不到"，提示改述 | — | v1 |

**"接得住"= 架构上接得住**：每个意图都走同一条管线（分类 → GoalTemplate → 计划 → 执行 → 安全网）。意图表可增长，新增意图 = 三处小改动（词表行 + GoalTemplate + prompt 描述），**引擎零改动**。v1 只实现第一梯队（BRING/DISTRACT/LOOKOUT/DELIVER + 已有 v0），其余是"词表里已注册、执行深度按里程碑补"——玩家说了"去买酒"，v1 的答案是"这我还做不了"（诚实拒绝），v2 就能跑。这与意图分类层的存在不矛盾：**分类永远接得住，执行深度逐版本加深**。
...
- **分工（who_does，通用机制）**：命令里每一件事都有明确执行人——底层语法是 §5.0 的**步骤级 actor 寻址**（每个步骤声明 `actor`，单随从 = 全部缺省 self，一带多 = 指定 actor 或 "all"），**所有 case 通用，角色互换只是分工的一个实例**（玩家和随从互换常规角色）。命令可声明"谁做哪件事"：case C = 偷（随从）+ 引开（玩家）——计划生成时按分工输出**两段**：随从的主动 plan（等窗口→偷→移交）+ 玩家的行动提示（"你只管缠住守卫，我听见动静就动手"）。玩家侧的执行不归引擎管（玩家是真人），引擎只负责：随从的 plan 里声明"窗口条件"（如守卫离开 watch_point），执行器检测到才推进。
...
public class GoalTemplate {
    ...
    public Func<WorldState, bool> Success;      // GOAL：成立 = 意图达成（一次性成功）
    public Func<WorldState, bool> Maintain;     // MAINTAIN（保持型意图）：达成后保持；掉线 = 预案
...
// ENGAGE:   Success(GOAL) = !seeing(目标, player) sustained && !moving(目标)
...
// GUIDE:     Success(GOAL) = distance(player, destination) < 3（目标参与者 = 玩家）；Impossible = 目的地 navmesh 不可达预检
// LOOKOUT:  Success = null（事件驱动：纯 MAINTAIN——异常即触发 signal_player，无限期待命，R6 豁免）
```

目标状态必须**引擎可验证**，所以不能是 LLM 自由文本——LLM 只能从词表选意图，目标状态由 C# 模板定义。这是"封闭语法"纪律的延伸（对齐现有 `tactic` 纪律）。
...
public class SceneSnapshot
{
    ...
    public List<ZoneInfo> Zones;           // 预定义场景锚点：door / meet_point…（watch_point/destination 不是预定义 Zone，按意图语义从目标物/命令解析坐标，见下）
...
| **动态目标引用** | query refs：`nearest_enemy(self)` / `all_in(zone)` / **`hidden_spot(near, min_dist)`**（运行时空间搜索：navmesh 可达 + 半径内无 agent + 不被 any eligible 目击者看见——case G"引到巷子"的落地：**mission 没有"巷子"标记**，自然语言位置描述 → 空间条件，由引擎找点）——目标引用运行时求值，不只快照实体 | **v2** | N/P、G |
...
```json
{
  "summary": "我去和守卫说门外有人找，把他引到门口，你趁机动。",
  "steps": [
    {"id": "s1", "action": "move_to",  "target": "guard", "within": 2.0, "timeout_s": 15},
    {"id": "s2", "action": "say_to",   "target": "guard", "text": "门外有人找你，好像有急事", "timeout_s": 8},
    {"id": "s3", "action": "wait",     "seconds": 2, "timeout_s": 5},
    {"id": "s4", "action": "move_to",  "target": "door", "within": 1.0,
        "until": {"type": "distance", "a": "guard", "b": "door", "op": "<", "value": 4}, "timeout_s": 25},
    {"id": "s5", "action": "say_to",   "target": "guard", "text": "就在门外等着呢，别让人等急了", "timeout_s": 6},
  ...
  "contingencies": [
    ...
    {"when": {"type": "distance", "a": "guard", "b": "watch_point", "op": ">", "value": 10, "sustained_s": 5}, "then": "s7", "one_shot": true},
    {"when": {"type": "following", "a": "guard", "b": "self", "op": "false"}, "then": "s8", "one_shot": true},
    {"when": {"type": "combat", "entity": "self"}, "then": "@abort_gracefully", "one_shot": true}
  ],
  "success_condition": {"type": "distance", "a": "guard", "b": "watch_point", "op": ">", "value": 10, "sustained_s": 5},
  "fail_condition":    {"type": "alert_phase", "entity": "guard", "phase": "Alarmed"}
}
```

> **示例计划的条件角色**（每个条件的位置即角色，§5.2 条件角色）：
> - s4 `until`（GATE）：守卫到门口（< 4m）→ 本步提前完成，去门外招呼
> - s6 `until`（GATE）：守卫离岗（> 10m sustained 5s）→ 窗口成立，推进到 s7 发动手信号
> - contingencies `when`（GATE）：警戒 Alarmed → @abort（FAIL 收尾）；离岗 10m sustained → 跳 s7；折返 `following==false` → 跳 s8（秘密消息警报 + s9 失败收尾）；combat → @abort
> - `success_condition`（GOAL）：守卫离岗 10m sustained = 计划成功
...
| 位置 | 角色 | 语义 | 例 |
|------|------|------|-----|
| 步骤 `until` | **GATE** | 条件成立 → 本步提前完成，推进下一步 | case A s4：守卫到门口即走 |
| 步骤 `when` | **GATE** | 条件成立 → 本步才放行（前置门控） | case C 偷窃步骤：`!seeing(any, self)` 才动手 |
| contingencies `when` | **GATE** | 条件成立 → 跳转（预案/警报/收尾） | case A 折返：`following==false` → s8 警报 |
| `success_condition` | **GOAL** | 条件成立 = 计划成功（收尾三路·成功） | case A：守卫离岗 10m sustained |
| `fail_condition` | **FAIL** | 条件成立 = 计划失败收尾 | 警戒 Alarmed → abort |
| GoalTemplate.Success + Maintain（保持型意图） | **GOAL + MAINTAIN** | 达成 → 进入保持期（持续成立）；掉线 = 预案；玩家 R3 叫停 = 收尾 | ENGAGE / COMMOTION / LOOKOUT |
| 计划 `triggers[]`（事件驱动意图专用） | **TRIGGER** | 条件成立（上升沿：false→true）→ signal_player 报告，**计划不结束**（报告后继续等待，可重复触发）；玩家 R3 叫停 = 收尾 | LOOKOUT 望风"有人来了！"（`in_zone(any, watch_zone)` 上升沿）/ SHADOW 盯梢目标停下/会面/离场 |

**执行器规则**：
- **GATE**：不成立 = 等待（步骤不推进）；超时 = 该步失败收尾
- **GOAL**：成立 = 收尾三路·成功
- **MAINTAIN**：达成后保持——条件翻转（掉线）→ 预案（再执行 / 报告玩家）；玩家 R3 新命令 → 收尾
...
Executing ──(安全网/预案命中)──▶ Paused（等待条件解除）
   │ 每 tick：预案 → 成功 → 失败 → 安全网（引擎级硬规则同帧求值；R5 对计划预期状态放行，见 §7.1——不是"先于计划条件"，是"与计划条件共享同一豁免判定"）
   │
...
- **until 提前推进**、**超时**（`timeout_s`，缺省 → `@abort_gracefully`）、**收尾统一三路一函数**（成功/失败/中断一个收口，发信号 → 清队列 → 释放控制 → 恢复默认行为）——沿用 `StealBarVM.CloseReason` 单一收口纪律。
- **报告双轨制**（收尾时如何告知玩家）：①**默认 = 返回当面说**——随从 abort 后恢复默认跟随（`FollowAgentAction(Leader)`，AgentBrain.cs:859-871）物理走回玩家身边，到达 ~3m 内触发一句冒泡报告（say_to + face，**转述实情原话**，情报有来源）再彻底收尾。适用：BRING 拒绝（Y2）/ DELIVER 传话 / SCOUT 侦察回报 / FETCH 取物失败等"随从本就该回到玩家身边"的收尾；②**特殊 = 即时秘密消息**（signal_player）——随从脱不开身的场景：LOOKOUT 望风 / SHADOW 盯梢（随从回不来，须留守）+ 紧急中断（R2 目标死 / R5 开战，随从可能在战斗或离玩家远）。**选型规则：收尾时随从按默认行为本来就回得来 → 当面说；回不来或紧急 → 即时消息。**
- **玩家干预**：执行中玩家对新 NPC 按 Plot → 旧计划作废（玩家最高优先级，自由感铁律）。
...
**方案**：台本在**计划阶段**与计划一起生成——LLM 为每个结算型步骤预写多轮对话台本，台词分支与结算结果枚举一一对应。执行期：播开场 → 引擎结算（瞬间）→ 播对应结果分支 → announce 汇报。**零 LLM，场面顺，结果与台词一致**。
...
```json
{
  ...
  "script": {
    ...
    "announce": "谈成了，{AMOUNT} 到手。"   // {AMOUNT} 由引擎按结算结果填充（PlaceholderResolver 模式）
  },
  "timeout_s": 15
}
```
...
1. **结果由引擎结算，台词只是包装**——LLM 写台本不能决定结果。目标的"让不让"由技能 vs 参数算出；台本分支必须覆盖引擎全部结果枚举（negotiate: success/partial/fail；duel: win/draw/lose），validator 校验缺分支 → 拒收该步骤。
...
**任何 NPC（守卫/村长/商人/路人）都可能是计划的相关方**。ReactiveAgent 是同一个引擎的两种用法：
...
**LLM 写剧本骨架，引擎按人格"演"强度**——防止"LLM 自说自话"（LLM 想让守卫蠢到跟走，但引擎参数不让）：

- 反应选择的确定性：weight 初值由 LLM 写（剧本感），引擎按人格修正（`duty` 高的 NPC，`follow_for_a_bit` 的 weight 下调、`refuse` 上调），取最高者为本次反应
- 反应强度/时长由人格决定：`follow_for_a_bit` 的跟随时长、`return_post` 的折返时机（`left_post_seconds(t)` 里的 t = 引擎按 duty 计算，LLM 给的 t 只是上限）
...
### 7.1 引擎级硬规则（所有计划共享，不写进任何计划，不特化任何场景）
...
| R3 | 玩家下达新命令（再次 Plot / `order_*`） | 旧计划作废（玩家最高优先级） |
...
**系统消息本地化**：Guardrail 的 abort 原因等**引擎生成**的消息走 `LWNTextHelper` + `std_*.xml`（铁律 13）；LLM 生成的 signal 文本是运行时内容直接显示。
...
| available | `InteractionMissionView.cs` `BuildAgentContext`(:564-650) | 条件：`brain.Leader == Agent.Main`（随从关系由 `FollowIntent` 建立，`GeneralIntents.cs:78-86`）。**引擎可驱动任意 NPC 当执行者，入口限随从是叙事选择**——v2 可放开到雇佣/临时伙伴/酒馆帮手，引擎零改动（这是入口与引擎不对称，属特性非缺陷） |
...
- **硬编码示例计划**跑通 case A 全链路：move_to→say_to→wait→move_to(door)→signal_player
...
| 结算台本缺结果分支 | Validator 拒收该步骤（分支必须覆盖引擎结果枚举） |
...
| LLM 写对手剧本自说自话（守卫被写得必跟走） | 人格演算修正 weight + 默认模板兜底 + duty 折返由引擎定 |

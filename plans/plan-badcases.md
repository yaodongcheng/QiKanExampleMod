...
> 🔴 **新（2026-09-10）：BC-008「随从把主公打晕」** → 修复计划 [命令系统后续优化.md](命令系统后续优化.md)（**待审批，未开工**；2 项必修 + 5 项该修，批准后按该文档第四节顺序施工）。
...
### BC-007 wait 步骤永不超时——内联分支 return 短路超时检查（2026-08-08 实机，执行器 bug，非 LLM）

- **【命令/场景】** BRING「把酒馆侍女带上来」（b5 wait 等 distance(侍女, player)<3 sustained 5s，侍女 22.6s 跟随时长到点折返后条件永假）
- **【日志证据】**（20:14 会话）
  ```
  20:14:05.763  ▶ 步骤 b5 开始（wait）
  20:14:29.736  侍女完成 ReactiveFollowAction → 折返回岗位
  → 之后零日志——b5 timeout_s 20s（25.7 到点）从未触发，卡死到 R6 总时长
  ```
- **【根因】** `TickCursor` 内联步骤分支（wait 的 WaitInlineState）**无条件 `return`**——572 行的步骤超时检查在 `return` 之后，**对所有内联步骤永不执行**。wait 步骤条件不成立时永远等不到超时（计划 1 的 t3 能完成只因条件恰好成立）。与 BC-004（tickDt 饿死）同族：计时路径被短路。
- **【修复】** 超时检查前移到 until 求值之后、子动作/内联驱动之前（`until 成立 → 完成优先；否则超时检查 → 再驱动`），对所有路径生效。
- **【回归】** 执行器层修复：BRING 失败路径应 20s 超时 → 跳 b7 fail 收尾（实机验证）；无独立回归用例
...
### BC-008 后置目标兜底误杀正确计划 → 随从把主公打晕（2026-09-10 玩家实机）

- **【命令/场景】** IM 私聊对随从孙娴淑下令「把他打晕」；回复轮 LLM 已正确指认目标 `大明卫所刀牌手#49`（就站在 5m 外）
- **【日志证据】**（玩家日志 `StoryEngine_RuntimeLog (9).txt`）
  ```
  20:55:38 [ReactiveRespond] 回包: {... "npc_action":"knockout", "action_target":"大明卫所刀牌手#49" ...}   ← 指认正确
  20:55:50 [ImCommandFlow] 目标纪律兜底 → 澄清卡已投递（候选 0 个）: 场上没找到「大明卫所刀牌手#49」这样的人——您说的是谁？
  20:56:48 [ImChat] Player → 孙娴淑: "你旁边的就是"                                        ← 玩家被迫补话
  20:57:03 大模型生成结果 {... "intent":{"intent_type":"KNOCKOUT","target":"player"} "steps":[k1 move_to player, k2 knockout player …]}
  20:57:13 [PlanExecutor] 孙娴淑 击晕了 朱祁心
  20:57:13 [ForcePlayAction] 朱祁心 'act_death_fall_front'                                 ← 主公真的倒地
  20:57:13 [Sense] 犯罪感知 Knockout: 「主公刚刚把人打晕了（温州府附近）」→ 4 名同场景队伍成员   ← 施暴者写反
  20:57:13 [Narration] 大明卫所刀牌手(Idx=49) 旁白: 我看见孙娴淑在偷窃                        ← 罪名判错
  ```
- **【根因】** 三层叠加：
  - ① **后置兜底拿命令原文重采候选**——命令原文「把他打晕」里没有人名（目标真身存 `ResolvedTargetText` 字段，同文件 330 行的 prompt 侧判据用的就是它，是对的）→ 0 命中 + "回复轮声明过目标" → 误判"场上没找到" → **扔掉正确计划 + 投出不成立的澄清卡**
  - ② **场景清单视角错位**——【场景当前人员】是**主公坐标**（`[player] 玩家：你身旁0米`、`你东侧5米`、`面朝玩家`）却注入**发给随从**的 prompt（同 prompt 写着"你是孙娴淑"）→ 同一个"你"指两个人 → LLM 把"你旁边的"认成 `[player]`
  - ③ **无友方目标守卫**——计划校验（只查结构/词表）与 KnockoutFlow（只判定结算）都不查敌我 → 照打
- **【派生副作用】** ⓐ `ReportPlayerMisconduct` 把施暴者**写死成"主公"** → 4 名随从记忆被写反（进【近期回忆】= 持久污染）ⓑ 目击者罪名分类读**受害者大脑**（`IsKnockedOut(victim)`；玩家永不建脑 → 恒 false）→ 一律落"偷窃"分支（旁白/警戒因素/质问台词全错）
- **【修复】** 见 [命令系统后续优化.md](命令系统后续优化.md)：P0-1（兜底判据改用已解析目标）/ P0-2（友方目标守卫，校验层 + 执行层）/ P1-3（清单坐标基准）/ P1-4（犯罪感知带施暴者）/ P1-5（罪名随事件传递）
- **【回归】** 同文档第六节 S1-S6 + R1（实机重放 + 日志断言）

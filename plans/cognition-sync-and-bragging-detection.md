...
|---|---|---|---|
| T05 | 大地图 | 先聊天气（无关数值）→ 再聊"钱袋多少" | 聊天气无【此刻现状】行；聊钱后注入；【对话历史】行带 `[N天前]` 前缀；回复引用旧值时答"那是几天前的账"（I1/I2/I5） | ⏳ 待测（✅ 已验半边：17:25"百草 我们身上多少钱"→ rag[gold] +【此刻现状】注入 + 答 9983996；🔧 已修触发词：`NumericStatusKeywords` 删「队伍/部队/人马」（NPC 回复高频泛称致历史检测误伤——"我们周围什么情况"误触发根因）+ 删「天气」（T05 要求聊天气零注入，天气走 time RAG 即时查）。待测：聊天气零注入 + 旧值引用契约"那是几天前的账"） |

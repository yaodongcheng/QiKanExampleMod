...
| # | 场景 | 操作 | 验证点（预期 + 日志/prompt） | 实测（2026-08-16） |
|---|---|---|---|---|
...
| T04 | 大地图 | 问"X 和 Y 关系咋样"（两队成员） | 关系等级词（挚友/交好/泛泛之交/面和心不和/仇深似海）+ 姻亲/同族/交战标记；随便聊有【主公的人缘】|rel|≥20；`[RelWeb]`（T/G10） | ⏳ 待测 |
| T05 | 大地图 | 先聊天气（无关数值）→ 再聊"钱袋多少" | 聊天气无【此刻现状】行；聊钱后注入；【对话历史】行带 `[N天前]` 前缀；回复引用旧值时答"那是几天前的账"（I1/I2/I5） | ⏳ 待测（⚠️ "我们周围什么情况"轮 prompt 出现了【此刻现状】——触发词待复查，见下） |
| T06 | 大地图 | ①随从声称"带你去吃面"类（决策 NONE）②"我一定小心"③下真实命令"去跟着商队" | ①前缀（吹牛）+ `[Bragging]`；②无前缀（收紧规则）；③无前缀（有执行路径豁免）（C） | ⏳ 待测 |
| T07 | 大地图→进城 | 队伍频道发消息前后对比 | 大地图名字无（城外）标记；进城镇场景后恢复（城外）（B） | ⏳ 待测 |
| T08 | 进城/藏身处/野战 | 进吕卡隆城 → 问"我们在哪" | `[Sense]` + 随从【近期回忆】"主公进了吕卡隆"；再问答"刚进的吕卡隆"（D 灭旷野案）；藏身处 → mission_hideout；野战 → mission_battle 带"双方投入约 N 余人" | ⏳ 待测 |
| T09 | 围城 | 随军围城一次 + 守城战一次 | mission_siege 文案"随军攻打" vs "抵御围攻"（攻守分流，禁误报）（D） | ⏳ 待测 |
| T10 | mission 内 | 把玩家血量打到 <0.35 | 随从 SpeechChannel 冒泡"主公挺住！"（`[Care]`）；回血 ≥0.7 再掉血可再触发；90s 冷却不重复（K1） | ⏳ 待测 |
| T11 | 犯罪 | 偷窃/击晕（有目击）→ 离开场景 | 同场景随从【近期回忆】记犯罪（`[Sense]`）；被抓现行冒泡"快走"（概率 0.5，`[Care]`）；离场后 40% 概率频道评论（`[ImEvent] crime`）；问"我犯过几次事"→【主公的成色】犯罪数增长（G3/K2/Q） | ⏳ 待测 |
| T12 | 犯罪被抓后 | 对守卫"饶了我吧" | 【受困处境】注入；pay_ransom/beg_mercy/bribe_guard 动作组；**玩家先开价被驳回/无视**（纪律）；认罚走 ComputeCost(Restitution) 统一入口（S） | ⏳ 待测 |
| T13 | 战斗 | 打完一仗（win/lose） | 随从【近期经历】"我随主公在 X 打了一仗"+击杀数（`[Narration]`）；群聊评论带情绪句（battle_lose 安慰口吻）（L/M） | ⏳ 待测 |
| T14 | 战斗/大事 | 攻城大捷（人数≥2×）→ 再刷日常进城×10 | 攻城大捷进【大事记】（`important` 白名单）；普通战斗/进城不进；刷完后大事记仍在（不 FIFO 挤出）；读档保留（N） | ⏳ 待测 |
| T15 | 分兵（重度） | 对随从"你带一队人跟着我"→ 批准卡片 | 独立 party 跟随主队（`[Party]` + 大地图）；分兵后问"我们在哪"→ 不注主队位置（L1 裁剪）；分兵随从感知 battle_win 照写（公开级）；"回来吧"→ 归队合并兵力归还（J） | ⏳ 待测 |
| T16 | 王国（重度，需身份） | 对自家国王"跟帝国开战吧"（影响力≥200） | propose_war 卡片 → 提案投票（`[Kingdom]`）；1-3 天后随从评论结果"议会通过/否决"（`[Kingdom]` + kingdom_decision，**禁止静默**）；影响力不足 → 国王拒绝文案；劝降敌领主 → persuade_join；命令自家领主 → order_march（R） | ⏳ 待测 |

**执行顺序**：T01-T07（第一轮大地图，无 LLM 依赖的确定性项先验）→ T08-T12（第二轮场景/犯罪）→ T13-T14（第三轮战斗）→ T15-T16（第四轮重度，可后测）。
**已知收尾**：全部通过后把用例状态回填到本表（✅/❌ + 现象），并登记轮子（已完成登记的 19 轮子 + R 决策结果广播增量）。

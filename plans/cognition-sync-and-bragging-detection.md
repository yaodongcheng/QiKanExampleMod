...
| T04 | 大地图 | 问"X 和 Y 关系咋样"（两队成员） | 关系等级词（挚友/交好/泛泛之交/面和心不和/仇深似海）+ 姻亲/同族/交战标记；随便聊有【主公的人缘】|rel|≥20；`[RelWeb]`（T/G10） | 17:23 实测："你俩关系怎么样"→ 斯唐纳夫 prompt 注入"泛泛之交（情谊 1）（同族）"——等级词 ✓ 同族标记 ✓ 无名指代兜底（你们俩=频道最近两人）✓；【主公的人缘】常态段多次出现 ✓；17:24 修复**当事人裁剪**（pair 含回复者本人放行，防阿速甘编"牢里同难之交"）——⚠️ 该修复未复验，顺手一句"你俩关系怎么样"即可；数据依赖项（随缘确认）：姻亲/交战标记需对应关系数据、【RelWeb】常态段需队伍 ≥2 NPC |
| T15 | 分兵（重度） | 对随从"你带一队人跟着我"→ 批准卡片 | 独立 party 跟随主队（`[Party]` + 大地图）；分兵后问"我们在哪"→ 不注主队位置（L1 裁剪）；分兵随从感知 battle_win 照写（公开级）；"回来吧"→ 归队合并兵力归还（J） | 18:03 首测：分兵成功 ✓（带走 9 兵）；L1 裁剪 ✓（分兵后 prompt 无【此刻处境（大地图）】【队伍物资】【此刻现状】）；L2 分兵见闻旁白 ✓（【近期经历】"我领了一队人马离了主队，跟着主公走"）；**18:36 修复后复测全绿**：①【分兵近况】注入 ✓（prompt 实锤"我正率部在 萨戈拉 附近/9 兵/率部跟随主公"）+ 问"你的队伍在干嘛"答"带这九名弟兄在萨戈拉附近的旷野里跟着您行军"（引用注入段不编）②跟随保持 ✓（`[Party] BuildSplitPartyAwareness` 三连读 `行为 EscortParty` 稳定，无逃跑）③**归队合并 ✓**——"百草你归队吧"→ LLM 回 `gather_to_player`（动作空间可见）→ `[Party] GATHER_TO_PLAYER 归队合并（归还 9 兵，主队现有 19）`；⏳ 未测：分兵随从感知 battle_win 照写（公开级）、敌人靠近不逃跑的长时间验证 |

> 📋 **会话实测备注（16:08-16:09 会话，T 表外）**：① quest 目标村修复生效——"目前有什么任务"答出"特维亚村里有百姓求援"（`rag[quest]`，注入"差事在 特维亚"）；② 被俘位置查询复验——"阿速甘在哪"答"被吕卡隆的守军拿住，关在牢里"；③ 家族频道点名在押随从——"阿速甘在吗"→ 阿速甘答"我被关在这吕卡隆的牢里"；④ "百草在哪"→ 斯唐纳夫答"在您身旁的旷野里，随队伍同行"（队伍成员点名 + 位置）。
> 📋 **会话实测备注（17:23-17:25 会话，T 表外）**：① 被俘认知复验——"有人吗"→ 阿速甘"我在吕卡隆的牢里"；② 复数强制跟随（"你俩"→ 跟随触发）✓；③ L4 账目边界——阿速甘"我们身上多少钱"→"身在牢中，账目不归我管，实不知"；④ 数值精确——斯唐纳夫"百草 我们身上多少钱"→"九百九十八万三千九百九十六枚"（9983996 ✓）；⑤ 17:24 发现两条幻觉链并修复：阿速甘"牢里同难之交"（当事人裁剪，已修）+ 斯唐纳夫"牢里隔着栏杆打过照面"（身份漂移，IM 纪律新增**在场纪律**，双语言 XML 已加）；⑥ market 主题兜底——"队伍眼下不在城中，物价无从查知"。
> 📋 **会话实测备注（18:03-18:07 会话，修复前基线）**：① T01/T02/T03 复验（吕卡隆 8.6、CampaignSight 3/7、全量注入段）✓；② T05 聊天气零注入 ✓（18:03:28"我这边天气怎么样"→ prompt 无【此刻现状】）；③ 口嗨检测两路径 ✓（声称≠决策观察日志 ×2 + （吹牛）前缀 ×1）；④ **发现两 bug 并修复**：分兵部队被 initiative flee 覆盖护航自行他往（修复：DoNotMakeNewDecisions 锁定跟随）+ "归队"→ move_to 玩家名降级 NONE 两连（修复：gather_to_player/party_patrol 补 attacker 侧 + move_to 玩家目标跟随兜底）；⑤ 18:04:43"您要细问，我这就给您念叨念叨"（吹牛）误伤（守卫表补"要"，已修待复验）。
> 📋 **会话实测备注（18:36-18:38 会话，修复后首测）**：① 分兵近况认知链全绿（见 T15 移入 ✅）；② **归队合并闭环**（gather_to_player 从"动作空间不可见"到"LLM 选中 → 执行 → 兵力归还"）；③ 新发现并修复：**分兵随从跟随主队时 IM 名字仍带（远处）标记**（18:38:24 玩家质问"明明在跟随我啊"）——方案 B 未覆盖分兵分支，`DescribeAwayLocation` 对不在主队 roster 的 Hero 一律报"远处"；**已改为真实距离标记**（跟随中「跟随中，距您约 N 里」/被派走「前往 X，距您约 N 里」，LWN_im_status_following_dist/marching_dist/far_dist 双语 XML）——待复验；④ 18:38:24 挑人误选在押阿速甘作答（远程加权），答得自洽但答非所问——挑人权重问题记录，非本次范围。

### ⏳ 未验证（验证通过后连同「实测现象」移入上方）

| # | 场景 | 操作 | 验证点（预期 + 日志/prompt） |
|---|---|---|---|
| T05 | 大地图 | 先聊天气（无关数值）→ 再聊"钱袋多少" | 聊天气无【此刻现状】行；聊钱后注入；【对话历史】行带 `[N天前]` 前缀；回复引用旧值时答"那是几天前的账"（I1/I2/I5） | ⏳ 待测（✅ 已验：17:25"百草 我们身上多少钱"→ rag[gold] +【此刻现状】注入 + 答 9983996；18:03"我这边天气怎么样"→ prompt 无【此刻现状】**聊天气零注入 ✓**（`NumericStatusKeywords` 删「天气」生效）；🔧 已修：time 主题补天气关键词（天气/晴/雨/雪/weather/rain/snow，G1 天气词此前写了但 rag 打不通）——**补词后"天气怎么样"应 `rag[time]` 命中，待复验**；待测：旧值引用契约"那是几天前的账"） |
| T06 | 大地图 | ①随从声称"带你去吃面"类（决策 NONE）②"我一定小心"③下真实命令"去跟着商队" | ①前缀（吹牛）+ `[Bragging]`；②无前缀（收紧规则）；③无前缀（有执行路径豁免）（C） | ⏳ 待测（✅ 已验：声称≠决策观察日志 ×2（18:03 split_party / move_to）+ （吹牛）前缀 ×1（18:04:43"我这就给您念叨念叨"）；🔧 已修：18:04:43 系条件承诺误伤（"要细问"条件式）——守卫表补单字"要"（匹配点前 4 字符窗口内），"我一定要去办"的"要"在匹配点后不受影响——**待复验：同句式不再（吹牛）+ 真实命令豁免**；待测：②"我一定小心"无前缀） |
| T07 | 大地图→进城 | 队伍频道发消息前后对比 | 大地图名字无（城外）标记；进城镇场景后恢复（城外）（B） | ⏳ 待测（🔧 18:38 发现并**统一规则重构**（用户裁定）：① 分兵随从跟随中显示「远处」误导 → 后缀统一分级规则：**玩家在 mission**＝同在 mission→「{N}米外」/ 本部队留守→城外·村外·堡外·据点外（按定居点类型，藏身处兜底据点外）/ 野外其他部队→「来自{N}米外的{部队}」/ 别的定居点→「来自{N}米外的{据点名}」/ 俘虏→「在{N}米外的{据点/押解部队}（俘虏中）」；**玩家在大地图**＝本队→「在队伍中」/ 不在本队→同 mission 规则；② **后缀与消息绑定**：结构化快照 `ImLocationSuffix{Kind,Name,Dist}` 存进 ImMessage（`ls` JSON 字段，发出时定格不回漂），UI 层 `ResolveLocationSuffix` 按当前语言 LWN_* 解析（换语言/改文案不动历史数据），旧消息回退实时计算；③ 距离统一（mission 内 Agent 米数；大地图 1 地图单位 ≈ 10 里 = 5000 米）；🔧 **21:06 用户裁定补**：大地图距离 ≥1000 米显示**公里**（40000米 → 40公里；显示层按数值换算，历史消息随时换单位，`LWN_im_dist_unit_km/m`）+ 缩略 IM 名字走 DisplaySenderName（ImChatCompact.xml 两处）+ 选中频道标题带人数（算上玩家，`LWN_im_title_members`）+ **留守处境段**（玩家在 mission、主队随从留守时注入【留守处境】——防 LLM 把主公位置当自己的，实机 21:06 百草答"我在吕卡隆城里"实际在城外）——**待复验：分兵跟随中 IM 名字显示「来自{N}公里外的百草的部队」、本队随从大地图显示「在队伍中」、进城后留守显示「城外/村外/堡外」、在押阿速甘显示「在{N}公里外的吕卡隆（俘虏中）」、城内 mission 时「被关押在吕卡隆」、留守随从答"我在城外候着"、缩略面板带后缀、标题显示（N人）**） |
...
| T10 | mission 内 | 把玩家血量打到 <0.35 | 随从 SpeechChannel 冒泡"主公挺住！"（`[Care]`）；回血 ≥0.7 再掉血可再触发；90s 冷却不重复（K1） | ⏳ 待测（🔧 2026-08-16 两轮重构：K1 由 OnMissionTick 每帧轮询改为**玩家受击事件链驱动**（用户裁定，复用 event_agent_damaged）——AttackTriggerMissionLogic.OnRegisterBlow 玩家受击 → **定向广播** 15m 内队伍成员（SendEventToAgent 逐个，args 带 InflictedDamage——blow 生成栈同步 ReceiveEvent 安全，OnAgentHit 的 HandleBlow 栈内同步调 brain 有 native 重入风险；不恢复全局范围广播——08-09 玩家脑 Suspend 裁定保持）→ AgentBrain event_agent_damaged 处理加血线关切分支（`victim==Agent.Main`；Health-damage 预估结算后血线；档位/冷却静态共享单喊，墙钟秒冷却跨 Mission 正确；跳过日志删除）；护主参战由既有 shouldHelp 逻辑自然触发——验证点不变：打残血看冒泡，触发时机 = 受击瞬间） |
...
## 二十、方案 P：玩家行为亲见（P2）— 🔴 **已删除（2026-08-16 用户裁定）**

> 🔴 **删除记录（2026-08-16）**：检测为降级近似（场景 tag + 玩家位置 ≤6m + 静止状态）——玩家
> 站在铁匠铺/酒馆区域发呆也会误报"正在打铁/喝酒"（站桩误报），且无可靠事件源替代（锻造/喝酒是
> 原版 UI 交互，mod 拿不到事件钩子）。错误记忆比没有更伤沉浸感 → 整个方案 P 删除（代码已随
> PlayerMissionEventLogic 并入/清理时移除，`CheckPlayerBehaviorSense` + `_behaviorSenseAt` 已删）。
> 若日后有可靠事件源（锻造完成事件等）再恢复，检测必须读真实状态而非位置推断。

> 原动机（2026-08-16 复盘 P2）：盘点表把锻造归"正确无知"——**配方/数值确实无知**（UI 私密数据），

...
**一句话现状**：**数据层已齐** —— 英雄 **1440** 人 / 家族 **296** 家 / 势力当主 **553** 格 / **一键体检 23 绿 / 1 红**（唯一的红 = **雷 53** 距离缓存，等 T5 据点进场景才能收尾）。
...
| 位置 | 规模 | 状态 |
|---|---|---|
| `TaikouHero.City_<年>` | 4 格 | ⏭ 仍挂（按日志为准该改，未动） |
| `TaikouHero.Kingdom_<年>` | 24 格 | ⏭ 仍挂（同上） |
| `TaikouHero.ClanID_<年>` | 66 格 | ⏭ 仍挂（疑 R5 亲属经**名字**解析；另有 460 格「已死亡者仍挂家族」） |
...
1. **上级日志是唯一信源** —— `Knowledge/太阁5/太阁日志/上级日志.md`（每年固定 1304 人；字段 `立场/势力/组织/上司/当主/据点/部下/陪臣`）。
...
| **0.8b** | **`ForceTaikou.csv` 退役删除**（快照口径=错误数据，日志口径为准） | 势力表只剩 `TaikouForce.csv`（读自己原地刷新）· **22 绿 0 红** |
...
**河野父子（袭名）最终处理（2026-09-12）**：父 `lord_tk5_906` 河野通直 / 子 `lord_tk5_300` 河野通直牛福丸。

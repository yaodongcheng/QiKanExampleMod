...
**现状一句话**：T1–T3 全通（建世界 → 建号 → 日本图 → 点京 → 进城中心），收尾全清；**补丁与防线战线收官**（能靠 XML + 离线检查替代的运行期补丁已清零）；**T4-a 英雄集合已定（2026-09-11，1112 行权威清单）**。**下一步 = T4-b 四表定义与交叉引用体检**（→ 见「五、」），再进 T4-c 全量铺数据。
...
- [~] **T4-a·补：英雄数据整备（亲属列 / 身份纠错 / 同人合并）** —— 见下方「四·补」：**主体已完成**（列改造 + 身份纠错 12 行 + 亲属补入 + 同人合并 1 对 + 新校验器）；剩 **柳生家三合一**（等点头）与 6 项待裁定
- [ ] **T4-b：⏭ 下一步 —— 四表定义与交叉引用体检** —— 见下方「五、四表体检」
...
**待办（勾选式）**：
- [ ] `Culture.csv` → `spcultures.xml`：定义自洽 + 被 clan/hero/settlement 引用是否全部命中
- [ ] `Kingdom.csv` → `spkingdoms.xml`：同上（含 `owner` 当主链）
- [ ] `Clan.csv` → `spclans.xml`：同上（含 `Owner`/`Kingdom`/`HomeSettlement` 三条链）
- [ ] `Settlements.csv` → `settlements.xml`：同上（含六年代 `Owner_<年>` 列与 `Clan.csv` 对账）
- [ ] **交叉闭合矩阵**：culture↔clan↔kingdom↔settlement↔hero 五者两两，谁引谁、断在哪
- [ ] 结论回填：断的引用 = 改数据还是补兜底；能离线检查的一律进 `run_all_checks.py`（纪律：每条检查必须有脚本兜底）

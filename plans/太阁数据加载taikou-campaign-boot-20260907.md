...
**现状一句话**：T1–T3 全通（建世界 → 建号 → 日本图 → 点京 → 进城中心），收尾全清；**补丁与防线战线收官**；**T4-a 英雄集合已定**；**T4-b 四表已清完**（据点 + 势力 + 家族 + 英雄年代格，2026-09-11）。
**⏭ 下一步 = 家族归属落地**（→ 见「零、」），做完即可进 T4-c 全量铺数据。

### 零、⏭ 下一步：家族归属落地（T4-b 收尾）

**结论先行**：**势力"存不存在"这一层已定死**；唯一没定的是 **"每个家族挂哪个王国"**。
...
| 🔴 **`Owner_<年>` 的语义** | **= 该年代此势力存不存在**（`-` = 不存在）。**生成某年代的 XML 时只看这一格**；值本身不要求是大名 |
| 存在性 | 一律沿用 `ForceTaikou.csv` 的 `-` 原标记（已与 Snr 快照六年代交叉验证）；漏标补正登记在生成器 `EXISTENCE_FIX`（只 1 条：上杉家 1554/1560） |
| 值 | 全部换成可解析的 `lord_tk5_*`（改前 386 格是织丰编号，其中 103 个连织丰 XML 里都没有） |
| 归并 | `ikko_shu`（一向宗）→ `honganji`（本愿寺），`Clan.csv` 7 行已同步 |

#### 0.2 🔴 家族侧 —— 唯一的待办

**引擎事实（已反编译核实，`Hero.MapFaction`）**：
```csharp
public IFaction MapFaction => Clan?.Kingdom ?? Clan;   // 英雄的阵营 = 家族的王国
```
→ **同一家族的所有人必然同属一个王国**，引擎**不允许**"人在家族 A、却效力王国 B"。所以"父子各为其主"在骑砍里**表达不了**，只能给家族选一边。

**归属规则（按优先级）**：

| # | 情况 | 挂谁 | 例 |
|---|---|---|---|
| 1 | **有当主** | 挂**当主**效力的那家 | 织田家 1598 → 织田秀信（`CareerStance=首领`）效力织田家 → 挂**织田家**；信雄投德川、有乐投丰臣是**个人去向**，不影响家族 |
| 2 | 没当主，但父子/同族各为其主 | 挂**辈分最高**那位 | 黑田家 1575 → 父职隆（当主）效力赤松 → 挂**赤松**；子官兵卫投织田属个人 |
| 3 | 成员归属一致 | 用那个（家臣家族） | 下间家→本愿寺、前波家→朝仓 |
| 4 | 成员当年都无主家 | **独立家族，不写 `super_faction`** | 柳生/服部/九鬼这类 |

**TODO 清单**：

| # | 事项 | 备注 |
|---|---|---|
| 1 | **`TaikouHero.csv` 加 `ClanID_<年>` ×6 列** | **只填有变化的那几格**（过继/改家：上杉景虎 1570 过继上杉、芦名盛重 1586 过继芦名），留空 = 同 `ClanID`；生成器取 `ClanID_<年> ?? ClanID`。⚠️ 这是**生成期输入**（喂"每年代一套 spclans 段"），不是运行时的东西 |
| 2 | **裁决 (b) 类剩的 20 格** | 山崎/稻叶/花房等逐个判：真·不同家的**分家**，父子/同族的**挂家长**（同名撞车的 57 个已自动拆完） |
| 3 | **`Clan.csv` 的 `Kingdom` 列退役** | 它记的是织丰的附庸关系（河野→mori、北畠→oda…），改为按 0.2 规则推导；删列前先确认无消费者 |
| 4 | **`Clan.csv` 的 `Owner` 列 204 条织丰编号** | 按"该家当主"重建（参照 `TaikouForce.Owner_<年>` 的做法） |
| 5 | **`Kingdom.csv` / `ForceTaikou.csv` 去留** | 它们是生成器的**输入**，删了生成器跑不动。二选一：**留着**（推荐）／删表+删生成器+TaikouForce 转正。另：`plans/scenario-campaign-mode/tools/GenerateXml.py:116` 读 `Kingdom.csv`（冻结的剧本工程） |
| 6 | **六时代段** | SubModule 现只注册 `TaikouCampaign`/1560/1582；六年代势力清单已算出 → `Knowledge/太阁5/骑砍2织丰角色ID对应/csv/_analysis/年代势力存在表_20260911.md` |

#### 0.3 本次会话（2026-09-11 下半场）产出

| 产出 | 脚本 |
|---|---|
| 势力表合并（135+185−1 → 185） | `Scripts/gen_taikou_force_csv.py` |
| 英雄表年代格修正（4 净改 + 2 登记例外） | `Scripts/fix_taikou_hero_kingdom.py` |
| 同姓家拆分（3 对手工 + **57 个自动**） | `Scripts/split_taikou_duplicate_clans.py` · `Scripts/split_taikou_conflict_clans.py` |
...
- [~] **T4-a·补：英雄数据整备（亲属列 / 身份纠错 / 同人合并）** —— 见下方「四·补」：**主体已完成**；剩 **柳生家三合一**（等点头）与 6 项待裁定。
      **2026-09-11 追加**：全表私用区字符就地还原 19 格（`fix_taikou_hero_pua.py`），列传里的 `U+F724`（=之）已不再漏进语言包
- [~] **T4-b：四表定义与交叉引用体检** —— **据点表 ✅ 完；⏭ 下一步 = Clan.csv 修正**（见「零、」与「五·补」）
- [ ] **T4-c：全量铺数据**（TK5 布局表 → `place_settlements.py` 批量 + `settlements.xml` 同步 + **兵种树/日式物品自建**——原料已备 112 SkillSet / 43 物品 / 25 BodyProperty；日式马/地痞外观留 T5）
...
**结论：`TaikouHero.csv` 就是权威英雄清单。** ⚠️ **现行数值 = 1111 行 × 130 列**（2026-09-11 亲属列改造 + 同人合并后；改造前是 1112 × 128）。全量铺数据从这份清单出发，不再有「可能有谁」的悬念。
...
| # | 事项 | 结果 |
|---|---|---|
| 1 | **列改造** | `FatherName/GrandFatherName/KinsName` → `FatherId/GrandFatherId/KinsId`（叫 Name 装的却是 ID）；新增 `MotherId`、`SpouseId`。列序：**父·母·配偶·祖父·亲戚**。128 → **130 列** |
| 2 | **身份纠错 12 行** | 1180 段女性行的 `EnglishName/ClanID/CultureID` 当年是**按名字硬匹配织丰**填的（重名即错）。已按铁证纠正：阿松→前田家、阿梅→北条家、阿玉→细川家、相模→武田家、小松→真田家、千代→山内家、归蝶→织田家、濑名/德公主→德川家、菊公主→上杉家、南姫/早川夫人→今川家。**家族/文化跟丈夫走** |
...
| 7 | **新校验器** | `check_englishname_clan_prefix.py`：EnglishName 家族名前缀 ↔ ClanID（+ 家族 id ↔ 自身罗马音第二关）。已进 `run_all_checks`（17 → 19 项） |
...
- [x] **`Clan.csv` / 势力表 ✅ 已完成（2026-09-11 下半场）** —— 经三条路清完：
      **①势力表合并** → `TaikouForce.csv`（`Kingdom.csv` × `ForceTaikou.csv`，见「零、0.1」）；
      **②同名撞车拆家** → 65 个家族拆出 57 个新家（`Scripts/split_taikou_conflict_clans.py`）；
      **③英雄表年代格修正** → 4 净改 + 2 登记例外（`Scripts/fix_taikou_hero_kingdom.py`）
      ⚠️ **仍未清**：`Clan.csv` 的 `Kingdom` 列（织丰附庸关系）与 `Owner` 列（204 条织丰编号）
      → 改由「零、0.2」的归属规则推导，见「零、」TODO 3/4
...
- 🔴 **`Clan.csv` 205 条 + `Kingdom.csv` 40 条 Owner 仍是织丰编号** → 就是「下一步」要清的那两条红

...
**现状一句话**：T1–T3 全通（建世界 → 建号 → 日本图 → 点京 → 进城中心），收尾全清；**补丁与防线战线收官**；**T4-a 英雄集合已定**；**T4-b 四表已清完**（据点 + 势力 + 英雄年代格，2026-09-11）；**✅ 家族已按侍奉关系重建**（278 家 × 六年代，2026-09-11，见「零、」）。
...
- **旧 Clan.csv 已存档** `csv/_archive/Clan_织丰口径_20260911.csv`，新表由生成器从零产出（278 行）。
...
| 面 | 动作 |
|---|---|
| `Clan.csv` | ✅ **整表重建**（278 行）：`ID \| ScriptName \| ChineseName \| LocozationName \| Surname \| Culture \| Is_Shokuho \| Owner_<年>/Kingdom_<年> ×6`；旧表存档 `csv/_archive/Clan_织丰口径_20260911.csv` |
...
| 产出 | 脚本 |
|---|---|
| 🔴 **家族按侍奉关系重建**（Clan.csv 278 行 + `ClanID_<年>`×6 + 据点 `Clan_<年>`） | `Scripts/gen_taikou_clan_csv.py`（新；`--check` 进一键体检） |

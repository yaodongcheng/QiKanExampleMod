...
**现状一句话**：T1–T3 全通（建世界 → 建号 → 日本图 → 点京 → 进城中心），收尾全清；**补丁与防线战线收官**；**T4-a 英雄集合已定**；**T4-b 据点表已完（2026-09-11，名字/年代名/别名/私用区全线清干净）**。
**⏭ 下一步 = T4-b 剩下的那半：Clan.csv 修正**（→ 见「五、」），再回头定 Kingdom/势力表口径，最后进 T4-c 全量铺数据。

### 零、⏭ 下一步：Clan.csv 修正（T4-b 下半场）

**一句话**：据点表这块已经干净了，家族表还剩两件事——

| # | 要做什么 | 现状 |
|---|---|---|
| 1 | **`Clan.csv` 的 `Owner` 列 205 条织丰编号 → `lord_tk5_*`** | 就是 `run_all_checks` 里 `check_taikou_world_tables` 那条红。映射表已有（981 条，从 git 旧版英雄表 + 据点对账两源合并），其中 125 条对得上真人；剩 80 条是织丰给「太阁5 里没这个人」的家族自动生成的占位当主 |
| 2 | **`Clan.csv` 自身自洽** | `Kingdom`/`Culture` 两条链**已全闭合** ✅；605 条 id 唯一 ✅；57 条无任何英雄/据点引用（多为无成员的家族，属数据缺口非错误） |

**同批要一起定的**（都卡在同一个决策上）：
- **`Kingdom.csv` 40 条 Owner**（同上）
- **势力表口径二选一**：`Kingdom.csv`（织丰 dump 135 条）还是 `ForceTaikou.csv`（TK5 势力总表 184 条，带 Warrior/Trader/Ninja/Pirate 分类）—— 已实测后者覆盖更全（英雄表全年代势力名 202 个，ForceTaikou 只差 17 个）
- **判据提醒**：家族 id 的规则已定 = **EnglishName 姓氏首块**（`clan_oda_1` ← `Oda`），由 `check_englishname_clan_prefix.py` 常驻守；改 Owner 时别动这条

> 完整的据点表沉淀（名字两个来源 / 三条不变量 / 身份→类型尺子 / PUA 码点 / Owner 迁移根因）→ 见「五·补」。
...
## 五、T4-b：四表定义与交叉引用体检（进行中 —— **据点表已完，下一步 = 家族表**）
...
**待办（勾选式）**：
- [x] **`Settlements.csv` ✅ 已完成（2026-09-11）** —— 见下方「五·补：据点表这一波做了什么」
- [ ] **⏭ 下一步：`Clan.csv` → `spclans.xml`** —— 定义自洽 + `Kingdom`/`Culture`/`Owner` 三条链闭合；
      **Owner 列 205 条还是织丰编号**（`lord_1_xxx`），是本轮唯一未清的红项，见下方「五·补」末段
- [ ] `Culture.csv` → `spcultures.xml`：定义自洽 ✅（21 条 id 唯一、必填齐）+ 被引用全部命中 ✅
- [ ] `Kingdom.csv` → `spkingdoms.xml`：⚠️ **方向待定** —— 势力清单的权威可能不在 Kingdom.csv 而在
      `ForceTaikou.csv`（184 条 TK5 势力总表，带 Warrior/Trader/Ninja/Pirate 四类型分类）；
      已实测：英雄表全年代势力名去重 202 个，ForceTaikou 覆盖到只剩 17 个缺口（旧口径 Kingdom.csv 差 68 个）
- [x] **交叉闭合矩阵** —— 已由 `Scripts/check_taikou_world_tables.py` 固化成常驻检查
- [x] 结论回填 —— 新 checker 已进 `run_all_checks.py`（19 → 20 项）

...
## 生成器自检的两级口径 —「生成器 bug」vs「数据缺口」（2026-09-11 登记）

**解决**：生成器带自检（值域/枚举/查名）时，**把数据缺口当 bug 硬报错 = 逼人去放宽标准**——
几次之后自检形同虚设；反过来漏检真 bug 又会让脏数据进世界。**必须区分「错了」和「还没有」。**

| 级别 | 判据 | 处置 |
|---|---|---|
| **硬错误**（exit 1） | 值不在**我们自己的**表里（如身份词不在 `IDENTITY_KEYS`）、ID 重复/为空、数值越界 | 生成器 bug，必须修 |
| **警告**（打印后继续，产物对应字段留空） | 引用**上游数据**里查不到（据点名不在 settlements、画像表无此人） | 数据缺口，不是生成器的锅；界面按「没有就不显示该行」处理 |

```python
# 范本：Scripts/gen_taikou_hero_catalog.py
if identity_raw not in IDENTITY_KEYS:
    problems.append(...)                     # 硬错误：这张表是我们自己维护的
if city and not lookup_seat(seats, city):
    warns.append(f"...查不到（留空）")        # 警告：上游可能确实还没建这个据点
```

**配套：查名要「先精确、再按已知的类型后缀归一」**——两张上游表对同一实体用不同写法是常态
（雷 58：`City_<年>` 写「奈良**之町**」而据点本名是「奈良」；太阁的类型后缀 = `之町/之砦/之里`）。

**同理：产出前扫一遍跨模块键引用**——内容包数据里出现基座前缀（`{=LWN_*}`）直接 fail（雷 57）。

**同族教训**：雷 55「读出来有值 ≠ 读对了」（值域断言）、雷 52「引擎盲读子节点，列表里禁夹注释」——
自检的价值不在「跑没跑」，而在**口径分得清**。

## 八、实体 ID 体系与别名表纪律（2026-09-11 登记，太阁英雄表实战产出）

> **场景**：内容包要给几百上千个实体（人/据点/势力）定 StringId，并维护一张「名字 → id」的别名表。
> **来源**：`TaikouHero.csv` 1112 行的编号重构（2026-09-11 一轮），一次实战踩出下面四条。

### 1. 先定「编号基准是哪一个」，再动手

同一个实体常有**多套编号**并存，且**只在低号段重合、高号段分道**。太阁5 就有三套：

| 编号 | 出处 | 特点 |
|---|---|---|
| DX 人物番号 | DX 游戏 `人物Ａ.人物番號` | 游戏的事实；**TaikouHero 行序就是它** |
| 原版人物番号 | 原版游戏同上 | 与 DX 在 **800 号起分道**（低段同号） |
| 外观ID / 世界表 pid | BUSTUP 立绘槽号 / Snr 快照槽 | **跑数据链要的那把**（年代数据、立绘都挂它） |

**纪律**：主 ID 只选一把（这里选 DX 人物番号 —— 它是游戏事实，且行序天然对齐），
其余各留**单独一列**备 join。三把钥匙并排放在表头前几列，一眼可比。
⚠️ 「低号段三套一致」**不能外推**到高号段 —— 实测 0–799 三套全等，800 起全乱。

### 2. 「同一实体占两行」的检测，禁止只查「ID 撞号」

**踩过**：一次重复检测只判「两行算出同一个 ID」→ 漏掉一对。
漏因：两行都落在「ID = 行号」的段里，**各拿各的号，永不撞号** —— 判据在结构上就覆盖不到。

**正解：用数据自身的强信号，两把一起上**
- ① 两行的**另一套编号**相同（如原版号同为 300）
- ② 两行（排除模板行）的**同一外键列**完全相同（如外观ID 单元都是 `300|1234`）
- ⚠️ 「外键列相同」对**模板行是正常的**（13 个身份模板共用同一立绘槽），必须排除

**发现的组一律登记成表**，未登记即 `exit 1`；发现后**不自动合并** —— 合不合是数据决策。

### 3. 「同名」可能是两个人：父子和袭名

**踩过**：把「河野通直」与「河野牛福丸」判成同一人（幼名/成年名），实际是**父子**。
**判据（纯数据，不靠史实知识）**：对同一槽的两版名字 A、B ——
**A 若在另一版日志的「别的槽」出现 → A 那个人另有位置 → 是两个人**；B 同理；都不出现 → 同人异名。
（实测：73 处「两版名字不同」里，**只有 1 处**是两个人，其余 72 处是同人异名。）

### 4. 别名表必须带「撞名闸门」，且判据分两级

**踩过两次**：① 儿子行残留父亲的别名；② 年代名无脑并入时，儿子 1582 年**袭父名**又撞回父亲。

| 级别 | 判据 | 处置 |
|---|---|---|
| 🔴 **别名键撞车** | 同一名字属于**另一行**的主名或别名 | `exit 1` —— 别名表是年代无关的全局表，撞了必然静默解析错人 |

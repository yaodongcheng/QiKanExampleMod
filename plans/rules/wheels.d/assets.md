...
> **来源**：换头工程（[Knowledge/蒂法换头工程.md](../../../Knowledge/蒂法换头工程.md)）+ 盔甲工程（[Knowledge/骑砍2盔甲资产工程.md](../../../Knowledge/骑砍2盔甲资产工程.md)）+ 战国无双换装（[plans/战国无双换装总纲.md](../../../plans/战国无双换装总纲.md)）三轮实战。
...
- 🔴🔴 **表情段的搬法必须是「法线投影 + 重心插值」，不能用最近邻加权**（2026-09-19 追加，亨利实机报告"下巴动但嘴张不开"）：
  最近邻 3 点加权会把源网格上的**折线**（"上唇不动 / 下唇下移"，源头唇线上下两点相距几毫米）抹成**渐变**
  → 目标上唇跟着下唇走 = 嘴唇不分开（实测唇/下巴比 0.34，原版 0.77）。换成投影后 0.52。
  四条配套（缺一条就出症状）：
  ① **`--proj-max` 按"两颗头的实际间距"给** —— 实测两头表面在嘴部**相距 7~13mm**（自建头常比原版前突 1cm+），
     给小了（6mm）= 嘴部全回退 = 白改。**量法**：`_probe_registration.py` 报逐区域"最近距中位"。
  ② **内部面判据**：同一 (x,z) 格子内 y 靠后 >3mm 的顶点（口腔内衬/下巴内侧/后脑）= 内部面 → 回退最近邻
     （内衬的**法线也朝前**，长距投影会打到前面的下巴上 → 撕尖刺）。
  ③ **1~2 轮轻平滑**（只作用表情段）：投影会让上下唇分界处相邻顶点落到源头不同三角形（各差 10~20mm）→ 唇缘撕裂。
  ④ 脸形段（1..59）保持原算法 —— 已验收，别顺手改。
  范本 = `transfer_channels.py --anim-map proj --proj-max 18 --anim-smooth 2`（配方固化在 `build_head_chain.py`）。
  验收：`check_chan_anatomy.py`（101 条全覆盖 + 地标 + 件级）+ 嘴部特写渲染对照（`_render_mouth.py`）。
...
4. 🔴 **判"凹陷/张嘴/口腔"必须用【有阴影】的渲染器**（2026-09-20 教训）：`Workbench` 无阴影 → 把**浅腔渲成一块发亮的平面**，"唇间那道暗缝"永远渲不出来，看着永远像"没张嘴"。为此在错误代理上连改三版数据（投影搬运 / 加深口腔 / 非线性增益），**实机没变化是因为判断基准本身是假的**。判这类问题用 **EEVEE + 一盏 SUN（方向 = 相机方向）+ 亮环境光**；只有判"贴图坏没坏"才用 Workbench 自发光那套。**判据**：换渲染器前后结论相反 = 之前的结论作废。
...
### 15.3c 🔴 第三种病：FBX 导出器把「导出那一刻的姿势」写成 bind pose（2026-09-24 定性；**修法未定 —— 下面那个"已验证"的尝试是错的，别照抄**）

> 🔴 **责任定性**：**源动画是好的**（实测源侧脚底 `0.000` 贴地）—— 这是**我们导出这一环造的 bug**，不是素材问题。

**现象**：clip 的 bind/rest 被写成**它自己的首帧姿势** ⇒ 套到官方骨架（rest `0.9145`）上整体抬高。
...
**「清 pose 之后再摘掉 action」（`tgt.animation_data.action = None`）**。
表面证据很好看：bind 确实修好了（rest 回官方值 `0.9145`）、脚底量出来 `0.0`。
**但导出的 FBX 里【姿态通道整个没了】→ 动画冻结在绑定姿势（T 字张开）**，
而且顺手弄丢**对象级位移**（带位移 clip 挂在骨架对象上的那份，实测 Root `3.67 m → 0`）。
带回查看器一看就露馅（用户 2026-09-24 当场抓到）。**已彻底回退**（代码 + 72 条产物 + 交付目录全部还原）。

🔴🔴 **连带的最重要教训：那次「验收通过」是假绿。**
只量了 `check_glb_feet.py` 的**脚底高度** —— 而**"冻结在绑定姿势"时脚底天然就是 0**，所以一路绿灯。
**判据必须两条一起过**：

| 判据 | 工具 | 阈值 |
|---|---|---|
| ① 关节有没有在动 | `check_glb_anim.py` | 最大转角 **100~180°**（≈0 = 冻结 / 搬运失败） |
| ② 脚底高不高 | `check_glb_feet.py` | **≈0.0**（`0.124` / `0.275` 都是这个病） |

#### 还没试的方向

**不动导出，改 `fbx_to_trf.py` 的旋转参照**：TRF 的旋转是「绝对局部变换」，而引擎是**按官方骨架的 rest** 去读它的；
现在 `fbx_to_trf.py` 用的是**输入 FBX 自己的 rest**（恰恰是被导出器写歪的那个）。
⇒ 把参照换成**官方骨架**（`input/target/bannerlord/human_lod_4.fbx`，按骨名取 `rest_local`）应当能修好，
而且**完全不碰导出的动画/位移通道**。落笔前先按上面两条判据验一条 clip，再铺开。
...
建模块四步、**从旧模块抽离资产的精确命令**（含 `Copy-Item` 掉层级的坑）、"编辑器打开就全好了"的三条原因、七条坑点速查。**本节只留要点，细节查那篇。**
...
> **「编辑器里看不出来、实机才发现」**，而代价是**白跑一轮 ModKit**（开编辑器 → 导入 → 挂材质 → Publish）。
> 所以：**进编辑器之前先把这张表过一遍**，比事后debug便宜一个数量级。
...
贴图文件名   = head_xxx_a_d / _n / _s
```

| 做的东西 | 材质名规则 |
|---|---|
| **自建头** | 脸壳用**裸名** `<头名>`；其余件加角色后缀 `<头名>_<角色>`，角色词固定 **`eye` / `mouth` / `lash` / `neck`** |
| **甲 / 武器 / 盾 / 其他** | 材质名 = **资源裸名**（与 FBX 内网格名、Item XML 的 `mesh=` 同名） |

🔴 **为什么不能改名**：编译后的后处理 `install_pack.py` 的 **`skinfix --fullmat` 按材质名判角色**
（`MatRole()` 只看名字里含不含 `mouth`/`lash`/`eye`，**不看子网格顺序**）。名字错或**重名** = 几件刷成同一个配方
= **眼睛和嘴糊上脸皮**，要到实机才看得出来。（同族坑：网格引用的材质名必须是**已定义**的，
否则编辑器拿默认白材质渲染 —— 铁律 32。）

**判据**：头 → `tools/sw2-pipeline/check_materials.py`（必备件齐 + 每个名字恰好一次 + 至多一个 `_neck`）；
甲/武器 → 材质名 == 资源名。**导入前先跑，秒级，纯 python。**

### 2. 贴图：三件套 + 槽位 + 通道约定

- 槽位：`tex[0]` = `_d`（漫反射）/ `tex[2]` = `_n`（法线）/ `tex[4]` = `_s`（金属·粗糙·AO）
- 🔴 **`_s` 的通道约定 = R 金属度 / G `255−粗糙度` / B 环境光遮蔽**
- 源的打包图**不能直接当 `_s` 用** —— 各家约定不同，喂错 = 金属感和粗糙度对调（画面全变但不崩）。
  已实测的一例：**CryEngine/KCD 的 `RMA` = R 粗糙 / G 金属 / B AO，跟骑砍正好对调** →
  转换用 `tools/kcd-pipeline/scripts/split_rma.py`（顺带拆出单独的金属度/粗糙度图，喂 `make_head_textures.py` 用）。
- **换新源第一件事：量通道顺序**，别照名字猜。做法 = 取几件**材质已知**的贴图看各通道均值
  （皮肤/布不导电 ⇒ 金属通道≈0；金属件 ⇒ 金属通道高）。

### 3. 贴图文件格式 —— 必须过 `png_for_editor.py`

进编辑器工程源的 PNG 必须是 **8bit RGB、只有 IHDR/IDAT/IEND**。
Blender 直出的是 **RGBA + 带 sRGB/gAMA/cHRM/eXIf/pHYs 附加块** → 实测编辑器会把**这张源图和它的编译产物一起删掉**。
`tools/face-pipeline/scripts/png_for_editor.py <in.png> [out.png]`（原地覆盖可省略输出）。

### 4. 编辑器材质开关 —— **每次重导 FBX 都要回看**

- 勾 **`Bumpmap` + `Skinning`**
- 🔴 **`skinning` 是【材质】属性，FBX 不携带** —— 漏了 = 网格**钉死在绑定姿势、完全不跟骨架动**
  （症状："头/甲大小位置全对，就是不跟动画"）。**静态件（武器、盾）不要勾。**
- 🔴 重导 FBX 会把材质的 **shader / flags / VertexLayout / 纹理槽全部刷回 FBX 默认值**
  —— "材质是独立资产、同名会复用"是**错的**。导完必须回看这四样。

### 5. 导入设置

**unit = m** / **不勾 Z-up** / **只勾 `Import meshes`**。

### 6. Publish 目标**选模块外**

选模块内 → 得到 `Modules/X/X/` 嵌套。发布后再把 `pack0.tpac` 拷进模块的 `AssetPackages/`。

### 7. 关卡 1（导出后、进编辑器前）—— `fbx_probe.py`

期望：`UnitScaleFactor = 100` / `UpAxis = 2` / **所有网格 Model 节点零缩放零旋转零平移**。
不合格直接点出是哪条不对，退出码非 0。**别跳过这步**（绕 X 翻 180°、放大 100 倍这类问题，这里一秒可见，实机就是半天）。

### 8. 模块资产目录的「编辑器 / 游戏」两态

引擎挑资产目录按固定顺序取**第一个存在的**（`Assets` → `AssetPackages` → …）。
`Assets/` 是编辑器的半成品（缺顶点流、贴图无像素）→ **游戏模式必须让 `Assets` 改名**（bat 化，见 CLAUDE.md 那一节）。

---

### 附：一条**不属于核对表但同源**的教训（源头侧）

换新源时，"源网格对象的变换"必须**烘进顶点**再往下走。
FBX 导入后**单位换算与转轴只写在对象矩阵里**，顶点本身是「源单位 + 源朝上轴」。

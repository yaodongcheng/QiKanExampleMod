...
玩家**不需要配置 LLM** 也能正常玩游戏。`Settings.Instance.IsLLMConfigured` 是所有 LLM 功能的总闸。
...
### 每条 LLM 代码路径必须在入口处检查 IsLLMConfigured
...
- **try-catch 是兜底不是替代**：不能用 try-catch 代替 IsLLMConfigured 检查

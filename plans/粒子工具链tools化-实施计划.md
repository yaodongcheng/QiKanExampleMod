...
## 七、追加：数据根 README 接入（2026-09-21）

**问题**：数据根 `D:\BrainMaker\骑砍2粒子特效复刻\README.md` 当时是**单摆的第二份文档** ——
在 D 盘施工顺手改了它，仓库这边不知道，两边必然分叉。隔壁 `tools/anim-retarget` 没这个问题：
它数据根的 `README.md` 是**符号链接**，指回仓库里的文档（同一份文件、两个路径）。

**改法（照 anim-retarget 办）**：

| # | 做什么 |
|---|---|
| 1 | D 盘那份独有的内容（数据目录注释树：每个子目录是什么、多大）并进工具链 `README.md` §1；顺手修掉它里面过时的「11 页预览」（现在是 99 页） |
| 2 | D 盘 `README.md` 换成**符号链接** → `tools/particle-pipeline/README.md`。从此只有一份，改哪边都是改这份 |

**为什么是符号链接不是 junction**：junction 只支持目录；跨盘符的**文件**要用符号链接
（`New-Item -ItemType SymbolicLink`）。

**验证**：`os.path.realpath('D:\...\README.md')` 解析到仓库那份（19141 字节，与仓库同步）。

**顺带**：`Debug/offline/README.md` 里那行「`particle_demo/` 只剩留底可删」已删 ——
目录早就没了，索引里留着是死条目。

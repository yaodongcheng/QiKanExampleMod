...
**坏消息**：亨利的脸**没有形变通道**（无 blendshape），表情靠 ~200 根 `mhenry_SSDR_*` 骨骼驱动。骑砍头要的是 59 条**顶点位移场**，跟骨骼动画是两套东西（见 §三 ②③⑥）。
...
| ② | **附属件带形变通道**，否则脸动它不动 | **要做**（用户裁定可捏脸）。做法 = 按最近邻 3 点反距离加权，把脸壳的位移场搬到附属件顶点 |
| ③ | **形状键顺序** Basis → 占位键 → KeyTime_1…59 | 同上；`transfer_channels.py` 已内置这个顺序 |
...
transfer_channels.py  把 59 条 KeyTime 位移场从蒂法头搬到亨利头（纯几何最近邻，源=蒂法 v10）

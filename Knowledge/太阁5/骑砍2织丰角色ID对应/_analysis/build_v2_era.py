...
def main():
    ...
    with open(os.path.join(DECODED, 'README.md'), 'w', encoding='utf-8') as f:
        ...
        f.write('## 数据引用导览（去哪里查什么）\n\n')
        f.write('六个剧本年代目录：`1554/ 1560/ 1568/ 1575/ 1582/ 1598/`（年代=剧本起始年）。\n\n')
        f.write('| 要查什么 | 路径 | 关键列 |\n')
        f.write('|---|---|---|\n')
        f.write('| 人物归属（某人当年属谁的势力、身份等级、上司、俸禄/野心/忠诚/家纹） | `{年代}/persons.csv` | person_id=人物号(character id)；force_id/force_name=所属势力；rank=身份等级；superior_id/superior_name=上司；salary/ambition/loyalty/kamon |\n')
        f.write('| 势力归属（某势力当主是谁、部众多少人） | `{年代}/forces.csv` | force_id/force_name=势力；lord_pid/lord_name=当主；member_count=成员数 |\n')
        f.write('| 城池归属（某城/町/砦当年城主、属哪家、兵粮金钱粮） | `{年代}/cities.csv` | city_idx=城序号(0-179城/180-245町/246-273里砦)；lord_name=城主；force_id/force_name=归属势力；soldiers/food/gold/train/morale |\n')
        f.write('| 人物号 ↔ 内存编辑器名单名（同号对照） | `../人物ID对照表_20260901.csv` | ID/character名单名/内存编辑器名单名 |\n')
        f.write('| 人物号 800-1056 三 AI 识别基准（claudecode 列为准） | `../../人物ID_820_1056_三方识别_20260831.csv` | id/豆包/ds/claudecode |\n')
        f.write('| 800-1056 段 × 6 年代提取（person 一览） | `person_era_info_20260901.csv` | 截图id/基准名/年代/世界表pid/force/rank/superior/... |\n\n')
        f.write('## 生成方法与校验\n\n')

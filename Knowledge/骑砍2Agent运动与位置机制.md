...
> 🔴 **更正（2026-09-21）——#5 的结论只覆盖「逻辑读者」，不覆盖「渲染读者」**：
> **「骨骼写不进渲染」是未验证状态，不是已证死的结论，别拿它当定论。** 两处证据链缺口：
> ① 该实验**没有调 `UpdateEntitialFramesFromLocalFrames()` / `ForceUpdateBoneFrames()`** —— 这两个 API 存在的意义就是「外部改了骨、强制重算」，不调则世界帧与蒙皮矩阵大概率还是旧的；
> ② 它当时看的是 **`riderZ` 没动**（骑手的**逻辑位置**），**不是「网格形不形变」**（**渲染读数**）。两件事从没分开验过。
> #5 真正证明的是「**骑手位置不从该骨实时姿势算**」——这条成立，但它推不出「骨写不进渲染」。
> 完整工具台清单、可行方案与 5 分钟验证做法 → [骑砍2骨骼级动画API与运行时姿势合成.md](骑砍2骨骼级动画API与运行时姿势合成.md)。
> ⚠️ 注意本表**对飞天/悬浮的结论不受影响**：那 9 条仍然全死，别重试。
...
- 绕过逻辑位置的改法（外观帧 #6、骨骼 #5）→ **值都改得动**；其中 #6 已确认渲染不读它（见 §6.3），**#5 的渲染侧未验**（见上方更正）。

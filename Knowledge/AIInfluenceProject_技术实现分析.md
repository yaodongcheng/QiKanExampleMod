...
> ⚠️ **概念参考，非可执行代码**
> 
> 本文是 AI 对 AIInfluenceProject 反编译代码的**结构分析报告**，文中的 C# 代码是**概念示意**，API 名称和签名可能与实际游戏 DLL 不一致。**不可直接复制粘贴使用**——具体 API 需以 `ilspycmd` 反编译确认为准。
> 
> 本文价值在于**设计思路和模块划分**的参考（如 NPC 主动性检查的计分模型、信息传播的延迟失真机制），而非代码实现。

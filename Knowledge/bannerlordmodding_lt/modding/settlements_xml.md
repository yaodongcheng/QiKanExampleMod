...
<!-- 源: https://docs.bannerlordmodding.lt/modding/settlements_xml/ | 抓取日期: 2026-09-09 -->
...
* ***gate\_posX*** : value is converted to a double, then cast to a float
* ***gate\_posY*** : value is converted to a double, then cast to a float
  ...
  + the coordinates are only updated if ***gate\_posX*** and ***gate\_posY*** exist in the xml
* ***port\_posX*** : value is converted to a double, then cast to a float
* ***port\_posY*** : value is converted to a double, then cast to a float
  ...
  + the coordinates are only updated if ***port\_posX*** and ***port\_posY*** exist in the xml
...
* ***is\_castle*** : used to check if a Town component belongs to a castle or a town (city)
  + (why yes, that is confusing nomenclature)
  + if omitted, defaults to `false`
* ***background\_crop\_position*** : parsed as a float
...
* ***background\_mesh*** : string that is usually for creating a file name used in widgets
...
* ***wait\_mesh*** : string to find the image via MenuContextHandler
  + default image for the encounter menu of the settlement
    - there's so many usages as BackgroundMesh that are unrelated to "waiting" that this is just "background\_mesh" for the encounter menu in the settlement
...
* ***level*** : the starting level for the building in that settlement as an integer
  ...
  + building\_settlement\_fortifications and building\_castle\_fortifications are the nodes which define what used to be the settlement's "level" attribute
...
* ***id*** : the StringId in MBObjectManager
  + generally follows the id of the parent node and is named in relation to the settlement it's bound to, eg. `village_comp_EW1_1` is the village component id for the settlement `village_EW1_1` which is in turn the first of 2 villages bound to town\_EW1
* ***background\_crop\_position*** : parsed as a float
  + used in EncyclopediaPages and Kingdom SettlementDecisions to change the cropping for the settlement image in the VM
* ***background\_mesh*** : string that is usually for creating a file name used in widgets
  + eg. EncyclopediaSettlementVM.FileName is set to `(BackgroundMeshName)_t`, and the widget's SpriteBrushLayerName is the resulting name for looking up the sprite
* ***castle\_background\_mesh*** : unsure where it's used
* ***wait\_mesh*** : string to find the image via MenuContextHandler
  + default image for the encounter menu of the settlement
    - there's so many usages as BackgroundMesh that are unrelated to "waiting" that this is just "background\_mesh" for the encounter menu in the settlement
* ***hearth*** : integer value used for the village on campaign creation
* ***village\_type*** : the id is looked up in MBObjectManager and the relevant object is stored on the settlement
...
* ***map\_icon*** : present in native xml, but seems unused
...
* ***gate\_rotation*** : present in native xml, but seems unused
  + this is present on hideouts, eg. `gate_rotation="6.283"`, but I see nowhere that this attribute is read, stored, or fetched, and because the entrance for hideouts is the hideout's position (because they have no defined gate\_pos for X and Y), then the rotation is effectively irrelevant
...
* ***complex\_template*** : the id is looked up in MBObjectManager and the relevant template is read from to fill out the dictionary of Locations for the LocationComplex
...
* ***scene\_name*** : used to define the scene that corresponds to a given upgrade level

...
<!-- 源: https://docs.bannerlordmodding.lt/modding/animations/ | 抓取日期: 2026-09-09 -->
...
Get TpacTools and export the animation to FBX, and then import to blender.  
You'll find info on https://docs.bannerlordmodding.lt/ about the correct import/export settings in Blender.  
It's going to need some clean up before you can edit it well because it has key frames on every frame.  
And make sure when you're importing the fbx back to Bannerlord to only import skeletal animations and not the skeleton.  
...
Right click in the resource browser and create an override (there's a bug if you try creating one that's not an override)  
...
float `blendInPeriod` - sets how fast animation should blend-in from the previous animation. Sometimes the best looking results to go to the .act\_none state are achieved with 1f or 2f values
...
When agent plays a looping animation set with SetActionChannel to stop it set it to act\_none action.
...
![](https://docs.bannerlordmodding.lt/pics/2602131324.png)

...
<!-- 源: https://docs.bannerlordmodding.lt/guides/custom_creature_skeleton/ | 抓取日期: 2026-09-09 -->
...
Do not give up on the first package that fails
...
Knowing the engine stores bones X-along does NOT mean exporting with `primary_bone_axis='X'`
...
There is no per-mesh bone palette limit
...
`primary_bone_axis` is not a preference, and the obvious test will not catch it
...
Do not ship a standalone skeleton-only tpac
...
A missing material binding does not present as a missing material

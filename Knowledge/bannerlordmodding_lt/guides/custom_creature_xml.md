...
<!-- 源: https://docs.bannerlordmodding.lt/guides/custom_creature_xml/ | 抓取日期: 2026-09-09 -->
...
`<Flags>` is only reassigned if you supply a `<Flags>` child element
...
Never declare `CanAttack` on a Mountable monster
...
A root-level `<action>` boots the client and kills a dedicated server
...
A custom `soln_*` id is silently ignored
...
Never add a second `soln_action_sets` row
...
`body_length` scales the RIDER too, not just the mount

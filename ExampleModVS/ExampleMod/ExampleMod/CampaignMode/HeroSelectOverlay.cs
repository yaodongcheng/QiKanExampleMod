...
{
	/// <summary>
	/// 🔴🔴 **【已停用 · 2026-09-11】本类不参与编译**（csproj 里已注释掉 Compile 行）——
	///   保留在磁盘上只是为了留住下面这些踩坑记录，**不是现行代码**。
	///
	///   为什么停用：选人整段挪到了建世界**之前**（改走 <see cref="HeroSelectScreen"/>）。
	///   本类存在的唯一理由是"世界建好那一刻还没有可用的屏/数据，所以要把层挂到 TopScreen 并每帧重试"——
	///   选人不再依赖活世界后，这个理由消失了，那套绕法（下面的注释详述了它踩过的坑）也就一并不要了。
	///
	///   实机验证新流程通过后可删本文件。设计见 plans/选人流程复刻太阁5-设计.md §十。
	/// </summary>

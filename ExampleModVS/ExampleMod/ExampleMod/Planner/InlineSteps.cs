...
namespace LivingWorldNpcs
{
    ...
    /// <summary>say_to：face 纪律 + 冒泡 + 可选 ask 广播（§4）。</summary>
    public class SayInlineState : IInlineStep
    {
        ...
        public SayInlineState(PlanExecutor executor, ActorCursor cursor, PlanStep step)
        {
            ...
            Ok = true;
            // 冒泡时长：按文本长度估算（"N 秒内必须播完"兜底由步骤 timeout 负责）
        ...
        public void OnTick(float dt)
        {
            ...
            if (_timer >= _duration)
            {
                if (!_broadcastDone)
                {
                    ...
                    // 通用 spoken_to 广播
                    if (_target != null)
                        AgentAIController.Instance?.SendEventToAgent(_target, "spoken_to", _agent);
                    if (_executor != null)

...
public sealed class Settlement : MBObjectBase, ILocatable<Settlement>, IMapPoint, ITrackableCampaignObject, ITrackableBase, ISiegeEventSide, IRandomOwner, ISettlementDataHolder
{
	...
	public Settlement FerryTarget { get; private set; }
	...
	public MBList<Ship> ShipStash { get; private set; }
	...
	protected override void AutoGeneratedInstanceCollectObjects(List<object> collectedObjects)
	{
		...
		collectedObjects.Add(ShipStash);
	}
	...
	internal static object AutoGeneratedGetMemberValueShipStash(object o)
	{
		return ((Settlement)o).ShipStash;
	}
	...
	{
		float num = _settlementWallSectionHitPointsRatioList[index];
		_settlementWallSectionHitPointsRatioList[index] = MBMath.ClampFloat(hitPointsRatio, 0f, 1f);
		if (num <= 0f && _settlementWallSectionHitPointsRatioList[index] > 0f)
		{
	...
	public void SetPortPosition(CampaignVec2 position)
	{
		...
		HasPort = position.ToVec2() != Vec2.Zero;
	}
	...
	internal void AddMobileParty(MobileParty mobileParty)
	{
		...
		{
			Debug.FailedAssert("mobileParty is already in mobileParties List!", "C:\\BuildAgent\\work\\mb3\\Source\\Bannerlord\\TaleWorlds.CampaignSystem\\Settlements\\Settlement.cs", "AddMobileParty", 680);
		}
	}
	...
	internal void RemoveMobileParty(MobileParty mobileParty)
	{
		...
		{
			Debug.FailedAssert("mobileParty is not in mobileParties List", "C:\\BuildAgent\\work\\mb3\\Source\\Bannerlord\\TaleWorlds.CampaignSystem\\Settlements\\Settlement.cs", "RemoveMobileParty", 692);
		}
	}
	...
	internal void AddHeroWithoutParty(Hero individual)
	{
		...
		{
			Debug.FailedAssert("Notable is already in Notable List!", "C:\\BuildAgent\\work\\mb3\\Source\\Bannerlord\\TaleWorlds.CampaignSystem\\Settlements\\Settlement.cs", "AddHeroWithoutParty", 717);
		}
	}
	...
	public void OnPartyInteraction(MobileParty engagingParty)
	{
		...
		else if (engagingParty.DefaultBehavior == AiBehavior.DefendSettlement && engagingParty.ShortTermTargetSettlement.IsVillage)
		{
	...
	internal void RemoveHeroWithoutParty(Hero individual)
	{
		...
		{
			Debug.FailedAssert("Notable is not in Notable List", "C:\\BuildAgent\\work\\mb3\\Source\\Bannerlord\\TaleWorlds.CampaignSystem\\Settlements\\Settlement.cs", "RemoveHeroWithoutParty", 787);
		}
	}
	...
	public override void Deserialize(MBObjectManager objectManager, XmlNode node)
	{
		...
		{
			CampaignVec2 portPosition = new CampaignVec2(new Vec2((float)Convert.ToDouble(node.Attributes["port_posX"].Value), (float)Convert.ToDouble(node.Attributes["port_posY"].Value)), isOnLand: false);
		...
		if (node.Attributes["ferry_target"] != null)
		{
			FerryTarget = (Settlement)objectManager.ReadObjectReferenceFromXml("ferry_target", typeof(Settlement), node);
		}
	...
	public void CheckPositionsForMapChangeAndUpdateIfNeeded()
	{
		...
		{
			Party.SiegeEvent.SetPositionAfterMapChange(GatePosition, PortPosition.IsValid() ? PortPosition : CampaignVec2.Invalid);
		}
		else if (Party.MapEvent != null)
		{
			...
			case MapEvent.BattleTypes.Hideout:
				Party.MapEvent.SetPositionAfterMapChange(GatePosition, PortPosition.IsValid() ? PortPosition : CampaignVec2.Invalid);
				break;

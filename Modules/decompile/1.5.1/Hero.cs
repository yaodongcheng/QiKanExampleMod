...
public sealed class Hero : MBObjectBase, ITrackableCampaignObject, ITrackableBase, IRandomOwner
{
	...
	[SaveableField(882)]
	private PartyConfiguration _partyConfiguration;
	...
	public bool CanJoinArmy
	{
		get
		{
			return _partyConfiguration?.CanJoinArmy ?? true;
		}
		set
		{
			if (_partyConfiguration == null)
			{
				_partyConfiguration = new PartyConfiguration();
			}
			_partyConfiguration.CanJoinArmy = value;
		}
	}

	public bool CanDonateTroopsToGarrison
	{
		get
		{
			return _partyConfiguration?.CanDonateTroopsToGarrison ?? true;
		}
		set
		{
			if (_partyConfiguration == null)
			...
			_partyConfiguration.CanDonateTroopsToGarrison = value;
		}
	}
	...
	public bool CanRaid
	{
		get
		{
			return _partyConfiguration?.CanRaid ?? true;
		}
		set
		{
			if (_partyConfiguration == null)
			...
			_partyConfiguration.CanRaid = value;
		}
	}

	public bool CanHaveFleet
	{
		get
		{
			return _partyConfiguration?.CanHaveFleet ?? true;
		}
		set
		{
			if (_partyConfiguration == null)
			...
			_partyConfiguration.CanHaveFleet = value;
		}
	}
	...
	protected override void AutoGeneratedInstanceCollectObjects(List<object> collectedObjects)
	{
		...
		collectedObjects.Add(_partyConfiguration);
	...
	internal static object AutoGeneratedGetMemberValue_partyConfiguration(object o)
	{
		return ((Hero)o)._partyConfiguration;
	}
	...
	public bool GetPerkValue(PerkObject perk, bool isPrimaryEffect, out float effectValue, BattleEnvironment environment = BattleEnvironment.Any)
	{
		if (_heroPerks != null && _heroPerks.GetPropertyValue(perk) != 0 && perk.ApplicableInEnvironment(environment, isPrimaryEffect))
		{
	...
	protected override void AfterLoad()
	{
		...
		bool flag = false;
		bool flag2 = false;
		ItemFlags p = (IsFemale ? ItemFlags.NotUsableByFemale : ItemFlags.NotUsableByMale);
		for (int j = 5; j < 12; j++)
		{
			EquipmentElement equipmentFromSlot = BattleEquipment.GetEquipmentFromSlot((EquipmentIndex)j);
			EquipmentElement equipmentFromSlot2 = CivilianEquipment.GetEquipmentFromSlot((EquipmentIndex)j);
			if (equipmentFromSlot.Item != null && equipmentFromSlot.Item.ItemFlags.HasAnyFlag(p))
			{
				flag = true;
			}
			if (equipmentFromSlot2.Item != null && equipmentFromSlot2.Item.ItemFlags.HasAnyFlag(p))
			{
				flag2 = true;
			}
			if (flag && flag2)
		...
		bool flag3 = !IsNoncombatant && !BattleEquipment.HasWeapon();
		if (BattleEquipment.IsEmpty() || BattleEquipment.IsEquipmentEqualTo(other) || flag || flag3)
		{
			Equipment equipmentForHeroComeOfAge = Campaign.Current.Models.EquipmentSelectionModel.GetEquipmentForHeroComeOfAge(this, Equipment.EquipmentType.Battle);
			EquipmentHelper.AssignHeroEquipmentFromEquipment(this, equipmentForHeroComeOfAge);
		}
		if (CivilianEquipment.IsEmpty() || equipment.IsEquipmentEqualTo(CivilianEquipment) || flag2)
		{
			Equipment equipmentForHeroComeOfAge2 = Campaign.Current.Models.EquipmentSelectionModel.GetEquipmentForHeroComeOfAge(this, Equipment.EquipmentType.Civilian);
			EquipmentHelper.AssignHeroEquipmentFromEquipment(this, equipmentForHeroComeOfAge2);
		}
	}
	...
	public void Heal(int healAmount, bool addXp = false)
	{
		if (!IsHealthFull())
		{
			int num = TaleWorlds.Library.MathF.Min(healAmount, CharacterObject.MaxHitPoints() - HitPoints);
			HitPoints += num;
			if (addXp)
			{
	...
	public bool CanBecomePrisoner()
	{
		...
		{
			Debug.FailedAssert("Only lords, companions and special quest heroes can become prisoners! Check CanBecomePrisoner usage.", "C:\\BuildAgent\\work\\mb3\\Source\\Bannerlord\\TaleWorlds.CampaignSystem\\Hero.cs", "CanBecomePrisoner", 2004);
			return false;
		}
		if (this != MainHero)
		{
	...
	public void AddDeathMark(Hero killerHero = null, KillCharacterAction.KillCharacterActionDetail deathMarkDetail = KillCharacterAction.KillCharacterActionDetail.None)
	{
		DeathMark = deathMarkDetail;
		DeathMarkKillerHero = killerHero;
		CampaignEventDispatcher.Instance.OnDeathMarkAdded(this, killerHero);
	}
	...
	public void ResetPartyConfiguration()
	{
		_partyConfiguration = null;
	}

...
public sealed class Kingdom : MBObjectBase, IFaction
{
	...
	[CachedData]
	private MBList<Town> _townsCache;
	...
	[SaveableProperty(4)]
	public TextObject FormalName { get; private set; }
	...
	public MBReadOnlyList<Town> Towns => _townsCache;
	...
	protected override void AutoGeneratedInstanceCollectObjects(List<object> collectedObjects)
	{
		...
		collectedObjects.Add(FormalName);
	...
	internal static object AutoGeneratedGetMemberValueFormalName(object o)
	{
		return ((Kingdom)o).FormalName;
	}
	...
	public void UpdateAlliedKingdoms()
	{
		...
		foreach (Kingdom item in All)
		{
			if (!item.IsEliminated && campaignBehavior != null && campaignBehavior.IsAllyWithKingdom(this, item))
			{
	...
	public Kingdom()
	{
		...
		FormalName = TextObject.GetEmpty();
	...
	public void InitializeKingdom(TextObject name, TextObject informalName, CultureObject culture, Banner banner, uint kingdomColor1, uint kingdomColor2, Settlement initialHomeSettlement, TextObject encyclopediaText, TextObject encyclopediaTitle, TextObject encyclopediaRulerTitle, uint? primaryBannerColor = null, uint? secondaryBannerColor = null)
	{
		ChangeKingdomName(name, informalName, encyclopediaTitle);
		Culture = culture;
		Banner = banner;
		Color = kingdomColor1;
		Color2 = kingdomColor2;
		PrimaryBannerColor = primaryBannerColor ?? Color;
		SecondaryBannerColor = secondaryBannerColor ?? Color2;
		PoliticalStagnation = 100;
		EncyclopediaText = encyclopediaText;
		FormalName = encyclopediaTitle;
	...
	public void ChangeKingdomName(TextObject name, TextObject informalName, TextObject title)
	{
		Name = name;
	...
	{
		return AlliedKingdoms.ContainsQ(other);
	}
	...
	public override void Deserialize(MBObjectManager objectManager, XmlNode node)
	{
		...
		FormalName = ((node.Attributes["title"] != null) ? new TextObject(node.Attributes["title"].Value) : TextObject.GetEmpty());
		...
		TextObject name = new TextObject(node.Attributes["name"].Value);
		TextObject informalName = ((node.Attributes["short_name"] != null) ? new TextObject(node.Attributes["short_name"].Value) : new TextObject(node.Attributes["name"].Value));
		CultureObject culture = (CultureObject)objectManager.ReadObjectReferenceFromXml("culture", typeof(CultureObject), node);
		uint kingdomColor = ((node.Attributes["color"] != null) ? Convert.ToUInt32(node.Attributes["color"].Value, 16) : 0u);
		uint kingdomColor2 = ((node.Attributes["color2"] != null) ? Convert.ToUInt32(node.Attributes["color2"].Value, 16) : 0u);
		if (GameTexts.TryGetText("str_kingdom_formal_name", out var _, base.StringId))
		{
			Debug.FailedAssert("str_kingdom_formal_name is not used anymore, title_name now used as formal_name since they are the same, consider changing that", "C:\\BuildAgent\\work\\mb3\\Source\\Bannerlord\\TaleWorlds.CampaignSystem\\Kingdom.cs", "Deserialize", 639);
		}
		InitializeKingdom(name, informalName, culture, null, kingdomColor, kingdomColor2, initialHomeSettlement, EncyclopediaText, FormalName, EncyclopediaRulerTitle);
	...
	public void OnFortificationRemoved(Town fortification)
	{
		...
		if (fortification.IsTown)
		{
			_townsCache.Remove(fortification);
		}
		foreach (Village boundVillage in fortification.Settlement.BoundVillages)
		{
